...
const gateId = 'GATE-REASON-REVISION-1-reasoning-revision-evidence-review';
const gateDir = path.join(platformRoot, 'reports', 'review-gates', gateId);
const screenshotDir = path.join(gateDir, 'screenshots');
const labPath = path.join('reports', 'review-gates', gateId, 'gate-playable-reasoning-revision-lab.html');
...
async function run() {
  ...
  const userDataDir = path.join(process.env.TEMP || process.env.TMP || platformRoot, `gate-reason-revision1-${Date.now()}`);
  ...
  try {
    ...
    const revisionFacts = await evaluateJson(
      cdp,
      sessionId,
      `({
        waterContextVisible: /Water kan in Nederland niet schaars zijn/.test(document.body.textContent),
        mode3SoftLabelVisible: /Redeneerketen ordenen/.test(document.body.textContent),
        visualFlowOverclaimAbsent: !/1\\.1\\.3 mode 3 - Stroomdiagram bouwen bridge/.test(document.body.textContent),
        replacementDeniedVisible: /vervangt de reasoning game niet/.test(document.body.textContent),
        contextBoxCount: document.querySelectorAll('[data-case-context]').length
      })`
    );
    proof.revision_facts = revisionFacts;
    proof.cases.push({ name: 'desktop-initial', state: initial });
    proof.screenshots.push({
      name: 'gate-reason-revision1-playable-initial',
      path: await screenshot(cdp, sessionId, 'gate-reason-revision1-playable-initial.png'),
    });
    ...
    proof.screenshots.push({
      name: 'gate-reason-revision1-playable-retry-feedback',
      path: await screenshot(cdp, sessionId, 'gate-reason-revision1-playable-retry-feedback.png'),
    });
    ...
    proof.screenshots.push({
      name: 'gate-reason-revision1-playable-next-action-focus',
      path: await screenshot(cdp, sessionId, 'gate-reason-revision1-playable-next-action-focus.png'),
    });
    ...
    proof.screenshots.push({
      name: 'gate-reason-revision1-playable-completed',
      path: await screenshot(cdp, sessionId, 'gate-reason-revision1-playable-completed.png'),
    });
    ...
    proof.screenshots.push({
      name: 'gate-reason-revision1-playable-mobile-dark-completed',
      path: await screenshot(cdp, sessionId, 'gate-reason-revision1-playable-mobile-dark-completed.png'),
    });
    ...
    proof.summary = {
      ...
      context_repair_proved: true,
      mode3_label_repair_proved: true,
      replacement_authority_denied: true,
    };
    ...
    if (!revisionFacts.waterContextVisible || !revisionFacts.mode3SoftLabelVisible || !revisionFacts.visualFlowOverclaimAbsent || !revisionFacts.replacementDeniedVisible || revisionFacts.contextBoxCount < 2) {
      throw new Error(`Revision facts failed: ${JSON.stringify(revisionFacts)}`);
    }
