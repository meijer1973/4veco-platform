...
const OUT_HTML = path.join(GATE_DIR, 'gate-playable-reasoning-revision-lab.html');
const OUT_DATA = path.join(GATE_DIR, 'gate-playable-reasoning-revision-data.json');
...
const CASES = [
  {
    ...
    paragraph: '1.1.1',
    mode: 0,
    title: '1.1.1 mode 0 - Stappen ordenen met zichtbare context',
    reviewIssue: 'Repaired context: the wrong student answer is visible before the ordering task.',
    contextTitle: 'Context en fout antwoord',
    context:
      'In de les zegt een leerling: "Water kan in Nederland niet schaars zijn, want er is meestal genoeg regen en kraanwater." Jij corrigeert dit met de economische betekenis van schaarste.',
    promptOverride:
      'Bouw de correctie op de leerlinguitspraak: water kan toch schaars zijn als behoeften groter zijn dan beschikbare middelen.',
    purposeOverride:
      'Kies alleen stappen die de gegeven leerlinguitspraak economisch corrigeren. De context staat hierboven zodat je niet hoeft te raden waar je op reageert.',
  },
  {
  ...
  },
  {
    ...
    paragraph: '1.1.3',
    mode: 3,
    title: '1.1.3 mode 3 - Redeneerketen ordenen bridge',
    reviewIssue: 'Repaired wording: this is an ordered-chain bridge, not visual flow-diagram construction.',
    contextTitle: 'Context en boundary',
    context:
      'Je leest een grafiek bij een prijs van EUR 9,00. Orden de redenering van gegeven prijs naar afgelezen hoeveelheid. Dit is keten-ordening; je tekent hier nog geen visueel stroomdiagram.',
    promptOverride: 'Orden de redeneerketen van prijsgegeven naar grafiekaflezing en conclusie.',
    purposeOverride:
      'Deze case bewijst dat keten-ordening speelbaar is. Een echte visuele flow-builder blijft vervolgwerk.',
  },
  {
...
function collectCase(config) {
  ...
  if (config.promptOverride) task.prompt = config.promptOverride;
  if (config.purposeOverride) task.purpose = config.purposeOverride;
  if (config.mode === 3) {
    task.skillLabel = 'Redeneerketen ordenen';
    if (task.interaction) {
      task.interaction.sequenceLabel = 'Jouw redeneerketen';
    }
  }
  ...
  return {
    ...
    contextTitle: config.contextTitle || 'Context',
...
function buildData() {
  return {
    ...
    status: 'review_only_playable_reasoning_revision_lab',
    title: 'GATE-REASON-REVISION-1 speelbaar reasoning revisielab',
    intro:
      'Gebruik de zichtbare taakcontrols en klik per case op Controleer case. Dit revisielab bewijst context- en labelreparaties; het vervangt de reasoning game niet.',
    cases: CASES.map(collectCase),
    revisionFocus: {
      previous_gate: 'GATE-REASON-STD-1',
      previous_reviewed_commit: '60a4fa0c6b3afe07643188499cb77c594a76011d',
      adoption_replacement_status: 'revise_required',
      local_practice_evidence_status: 'accepted_with_flags',
      fixes_proved_here: [
        'water scarcity task has visible wrong-answer context',
        'mode 3 is labelled as reasoning-chain ordering rather than visual flow-diagram construction',
        'review lab states these tasks cannot yet replace the reasoning game',
      ],
    },
    heldLanes: [
      {
    ...
    boundaries: {
      ...
      reasoning_game_replacement_authorized: false,
    },
  };
}
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .context-box { border:1px solid color-mix(in srgb, var(--accent) 35%, var(--line)); background:color-mix(in srgb, var(--accent) 8%, var(--panel)); border-radius:8px; padding:12px; margin:12px 0; }
    .context-box strong { display:block; margin-bottom:4px; }
...
<body>
  <main>
    ...
    <section class="gate-panel" id="review-only">
      ...
      <p>Een case telt pas mee wanneer de task-shell engine een correcte eindstaat geeft. Voor mode 5 telt alleen de zelfcheck-eindstaat, niet een inhoudelijke beoordeling. Dit lab is adoptievoorbereiding en bewijst niet dat de huidige tasks het volledige reasoning game kunnen vervangen.</p>
    </section>
          ...
          return '<section class="gate-case" data-case-id="' + escapeHtml(entry.id) + '">' +
            '<div class="gate-case-head">' +
              '<div>' +
                ...
                (entry.context ? '<div class="context-box" data-case-context><strong>' + escapeHtml(entry.contextTitle || 'Context') + '</strong><span>' + escapeHtml(entry.context) + '</span></div>' : '') +
                '<div class="case-meta">' +
