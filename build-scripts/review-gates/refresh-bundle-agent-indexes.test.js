...
function validateRefreshHelperNavigation(input) {
  const failures = [];
  const requireValue = (condition, id) => { if (!condition) failures.push(id); };
  requireValue(input.researchMap.includes(`](${refreshHelperPath})`), 'map.helper_link');
  requireValue(input.researchMap.includes('](docs/review/pr-integration-lane-policy.md)'), 'map.integration_policy');
  requireValue(input.githubEntry.includes('](RESEARCH_AGENT_MAP.md)'), 'github_entry.map');
...
describe('trusted bundle agent-index refresh', () => {
  test('keeps the trusted refresh helper discoverable through the map, policy and generated URL index', () => {
  ...
  test.each([
    ['map.helper_link', 'researchMap', '', null, `](${refreshHelperPath})`],
    ['map.integration_policy', 'researchMap', '', null, '](docs/review/pr-integration-lane-policy.md)'],
    ['github_entry.map', 'githubEntry', '', null, '](RESEARCH_AGENT_MAP.md)'],
