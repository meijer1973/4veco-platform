...
Platform-owned revision of the received assembler. Source ownership is
documented in EXERCISE-ROUTES.md. The immutable received script remains historical. Assembly does not overwrite back.md.
"""
...
def main():
 ...
 for b in ['3','4']:
  ...
  for c in chaps:
   ...
   rendered=build_chapter(c,('student',),offset)
   if any(r['overflow'] or r['pages'] != len(a['pages']) for r in rendered):
    raise RuntimeError('Book-page chapter overflow '+c)
   offset+=len(a['pages'])
