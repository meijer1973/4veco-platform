...
def verify(root):
    ...
    require({r['path']for r in manifest['files']}=={config['cover']['preview'],*(b['output']for b in config['bundles'])},'Unexpected output inventory')
    ...
    accepted=json.loads((Path(__file__).resolve().parents[2]/'reports/review-gates/book2-theory-signed-20260921/accepted-navigation-comparison.json').read_text(encoding='utf8'))
    ...
    for index,b in enumerate(config['bundles']):
        with fitz.open(root/b['output']) as final:
            ...
            require('PDF-pagina' in final[1].get_text(),'Missing page-number convention')
            ...
            links+=current_links;bundles.append({'kind':b['kind'],'pages':len(final),'links':current_links,'bookmarks':len(final.get_toc())})
    ...
    return {'revision':REVISION,'passed':True,'bundles':bundles,'identical_assembled_chapter_pages':chapters,'links_checked':links,'overview':overview,'cover_geometry':'PASS','paragraph_exports':verify_exports(root)}
