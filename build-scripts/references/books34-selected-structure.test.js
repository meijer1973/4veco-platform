...
test('current identity, complete preservation and optional paired checkout pass',()=>expect(verify().failures).toEqual([]));
test.each([
...
test('lookup uses current selected subjects and version identity',()=>{
 expect(lookupParagraph('3.3.1').source_path).toContain('book-3-outline.md');
 expect(lookupParagraph('4.1.1').title).toMatch(/monopol/i);
 expect(lookupParagraph('4.3.6').kind).toBe('gemengde_opgaven');
 expect(()=>lookupParagraph('4.3.7')).toThrow();
});
