...
const {sha,PACKAGE,REVISION}=require('./migrate-books34-v3');
function safeFile(root,name){
 if(typeof name!=='string'||!name||name.includes('\\')||name.includes(':')||name.split('/').some(p=>!p||p==='.'||p==='..')||path.isAbsolute(name))throw new Error('Unsafe source path '+name);
 const file=path.resolve(root,name),real=fs.realpathSync(file),base=fs.realpathSync(root);
 if(!real.startsWith(base+path.sep)||!fs.statSync(real).isFile())throw new Error('Escaping/non-file source '+name);
 return file;
}
function consumeTarget(record,lessons=path.resolve(__dirname,'../../../4veco-lessen')) {
 ...
 const root=path.join(lessons,PACKAGE);
 ...
 return {context:t.context,context_html:html,subquestions:structuredClone(t.subquestions),sources:structuredClone(t.sources),figures_consumed:used,tables_consumed:tables,source_locator:structuredClone(loc)};
}
