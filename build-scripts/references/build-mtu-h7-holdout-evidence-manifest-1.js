...
const OUT_JSON = 'reports/mtu-hardening/mtu-h7-holdout-evidence-manifest-1.json';
const OUT_MD = 'reports/mtu-hardening/mtu-h7-holdout-evidence-manifest-1.md';
const EVIDENCE_DIR = 'reports/mtu-hardening/mtu-h7-holdout-evidence-manifest-1-evidence';
...
function build() {
  ...
  const holdoutRecords = (curator.selected_records || []).filter((record) => record.split === 'locked_holdout');
  const generatedAt = new Date().toISOString();
  fs.rmSync(repoPath(EVIDENCE_DIR), { recursive: true, force: true });
  const records = holdoutRecords.map((record) => {
    ...
    return {
      ...
      evidence_state: 'locked_holdout_official_prompt_and_correction_pages_rendered_after_method_freeze',
      source_locator: {
  ...
  const manifest = {
    schema_version: 1,
    sprint_id: SPRINT_ID,
    manifest_id: 'mtu-h7-holdout-evidence-manifest-1',
    status: 'locked_holdout_official_evidence_rendered_after_method_freeze',
    generated_at: generatedAt,
    source_curator_view: CURATOR_VIEW,
    diagnostic_records_rendered: 0,
    locked_holdout_record_count: records.length,
    records,
...
function renderMarkdown(manifest) {
  ...
  return `# MTU H7 Holdout Evidence Manifest 1

Status: \`${manifest.status}\`

Diagnostic records rendered: ${manifest.diagnostic_records_rendered}

Locked holdout records rendered: ${manifest.locked_holdout_record_count}

No H7 repair, lesson output, protected-reference mutation, product route, or student/product use are authorized.
...
try {
  ...
  console.log(`OK ${SPRINT_ID}: rendered holdout evidence for ${manifest.records.length} records`);
} catch (error) {
