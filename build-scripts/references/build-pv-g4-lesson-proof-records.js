...
const GATE_CLOSURE = path.join(
  PLATFORM_ROOT,
  'reports',
  'review-gates',
  'GATE-PV-G4-lesson-regression',
  'gate-closure.json'
);
...
function buildHcsPacket({ proof1, proof2, contractReportPath, a61ReportPath }) {
  ...
  const statusLine = gateClosure ? `gate_reviewed_${gateClosure.status}` : 'submitted_for_human_review';
  const lines = [
    ...
    '',
    gateClosure
      ? `PV-G4 has HCS gate status \`${gateClosure.status}\`. Use this packet as the lesson-side proof evidence record and carry the gate conditions forward.`
      : ready
    ...
    ...(gateClosure ? [
      ...
      `- Closed on: \`${gateClosure.closed_on}\``,
    ...
    gateClosure ? `- HCS gate outcome is recorded as \`${gateClosure.status}\`; the conditions remain active.` : '- HCS must still decide the gate outcome.',
    '',
    '## Validation Status',
    '',
    '| Command | Status |',
