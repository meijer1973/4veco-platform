...
const MANIFEST = 'reports/mtu-hardening/mtu-h7-holdout-evidence-manifest-1.json';
...
function validate() {
  ...
  if (manifest.status !== 'locked_holdout_official_evidence_rendered_after_method_freeze') {
    failures.push(`status mismatch: ${manifest.status}`);
  }
  if (manifest.locked_holdout_record_count !== 8 || (manifest.records || []).length !== 8) {
    failures.push('holdout evidence manifest must contain exactly 8 locked-holdout records');
  }
  if (manifest.diagnostic_records_rendered !== 0) {
    failures.push('holdout evidence manifest must not render diagnostic records');
  }
...
if (process.argv.includes('--json')) {
  ...
  console.log(
    `OK ${SPRINT_ID}: holdout evidence manifest rendered ${result.summary.records} records (${result.summary.prompt_pngs} prompt PNGs, ${result.summary.correction_pngs} correction PNGs)`
  );
} else {
