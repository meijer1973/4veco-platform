#!/usr/bin/env node
/**
 * Capture MATH-UX-2 math/task-shell screenshots through headless Chromium.
 *
 * HOW TO ADAPT:
 * - Keep captures tied to proof requirements in MATH-UX-2.
 * - Prefer named evidence cases over broad screenshots.
...
const bookRoot = path.resolve(
  process.env.MATH_UX2_BOOK_ROOT || path.join(platformRoot, '..', '4veco-lessen', 'Boek 1 - Grondslagen, vraag en aanbod')
);
const outputDir = path.resolve(platformRoot, 'reports', 'sprints', 'MATH-UX-2-screenshots');
const chromeExe = process.env.CHROME_EXE
  || 'C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe';

const cases = [
  { name: 'desktop-light-112-math-task-shell', theme: 'light', size: { width: 1280, height: 760 }, action: 'start-a38' },
  { name: 'mobile-light-112-math-route-first', theme: 'light', size: { width: 390, height: 844 }, action: 'route-only' },
  { name: 'desktop-dark-112-math-task-shell', theme: 'dark', size: { width: 1280, height: 760 }, action: 'start-a38' },
  { name: 'mobile-dark-112-math-feedback', theme: 'dark', size: { width: 390, height: 844 }, action: 'wrong-answer' }
];
...
function pageUrl(serverPort) {
  const file = '1.1 Hoofdstuk Economisch denken en rekenen/1.1.2 Percentages en indexcijfers/1.1.2 Percentages en indexcijfers \u2013 wiskundevaardigheden.html';
...
async function main() {
  ...
  const profileDir = path.join('C:\\tmp\\Codex-work', `math-ux2-chrome-${Date.now()}`);
  ...
  try {
    ...
    for (const item of cases) {
      ...
      const proof = await cdp.send('Runtime.evaluate', {
        expression: `(() => {
          ...
          const shell = document.querySelector('[data-skilltree-task-shell="MATH-UX-2"]');
      ...
      if (item.action !== 'route-only' && (!value.shellText || !value.taskFamily)) {
        throw new Error(`${item.name}: missing task-shell proof`);
      }
      if (item.action === 'wrong-answer' && !value.feedbackState) {
        throw new Error(`${item.name}: missing feedback proof`);
      }
    ...
    console.log(`Captured ${manifest.length} MATH-UX-2 screenshots in ${outputDir}`);
  } finally {
    chrome.kill();
    server.close();
    await sleep(500);
