...
const bookRoot = path.resolve(
  process.env.SHORT_CHECK_BOOK_ROOT ||
...
const proofJsonPath = path.join(platformRoot, 'reports', 'json', 'short-check-workbench-112-rendered-1-proof.json');
...
const cases = [
  ...
  { case: 'desktop-light-local-success', action: 'local-success', size: { width: 1280, height: 900 }, theme: 'light' },
  { case: 'desktop-light-completed-advisory', action: 'complete', size: { width: 1280, height: 900 }, theme: 'light' },
  { case: 'mobile-light-initial', action: 'initial', size: { width: 390, height: 844 }, theme: 'light' },
  { case: 'mobile-light-local-success', action: 'local-success', size: { width: 390, height: 844 }, theme: 'light' },
  { case: 'mobile-dark-initial', action: 'initial', size: { width: 390, height: 844 }, theme: 'dark' },
  { case: 'mobile-dark-local-success', action: 'local-success', size: { width: 390, height: 844 }, theme: 'dark' },
...
async function waitForRoute(cdp, sessionId) {
  ...
  throw new Error('Timed out waiting for 1.1.2 short-check Golden route');
}
...
async function driveAction(cdp, sessionId, action) {
  ...
  const script = `(() => {
    ...
    function choose(id, optionId) {
      click(step(id).querySelector('[data-ge-choice-option][data-option-id="' + optionId + '"]'));
    }
    ...
    if (action === 'wrong-retry') {
      choose('oude-waarde-als-basis', 'b');
      checkStep('oude-waarde-als-basis');
    } else if (action === 'local-success') {
      choose('oude-waarde-als-basis', 'a');
      checkStep('oude-waarde-als-basis');
    } else if (action === 'complete') {
      choose('oude-waarde-als-basis', 'a');
      choose('index-zonder-procent', 'b');
      choose('indexpunten-niet-procent', 'a');
  ...
  return evaluateJson(
    ...
    `(() => {
      ...
      const taskStates = Array.from(document.querySelectorAll('[data-task-id]')).map((node) => {
        ...
        return {
          id,
          selected: node.querySelector('[data-ge-choice-option][aria-pressed="true"]')?.getAttribute('data-option-id') || null,
      ...
      return {
        ...
        variant: window.EXIT_TICKET_DATA?.layout?.variant || null,
        surface: window.EXIT_TICKET_DATA?.surface || null,
        targetEquivalentCandidate: window.EXIT_TICKET_DATA?.targetEquivalent?.candidate,
        ...
        advisoryIntent: window.EXIT_TICKET_DATA?.advisory?.intent || '',
        hintsAbsent: window.EXIT_TICKET_DATA?.advisory?.hintsAbsent === true,
        ...
        choiceTaskCount: document.querySelectorAll('.ge-step-choice').length,
        choiceOptionCount: document.querySelectorAll('[data-ge-choice-option]').length,
        ...
        lockCount: document.querySelectorAll('.ge-locked').length,
        feedback,
        taskStates,
        ...
        firstTaskGood: taskStates.some((item) => item.id === 'oude-waarde-als-basis' && item.feedbackTone === 'good'),
        firstTaskRetry: taskStates.some((item) => item.id === 'oude-waarde-als-basis' && item.feedbackTone === 'warn'),
        allTaskFeedbackGood: taskStates.length === 3 && taskStates.every((item) => item.feedbackTone === 'good'),
        completionVisible: document.querySelector('[data-ge-completion]')?.classList.contains('is-visible') === true,
        routeLinks: Array.from(document.querySelectorAll('.ge-route-pill, .ge-choice-route a')).map((link) => link.getAttribute('href')),
        viewport: { width: window.innerWidth, height: window.innerHeight },
        overclaimVisible: /(eindopgave|aankunt|bewezen|aangetoond|beheerst|diagnos|mastery|summatief|Scale Gate 1|productgebruik|target-equivalent|doel gehaald)/i.test(bodyText)
      };
    })()`
  );
}
...
function proofMarkdown(generated, localUrl, captured) {
  return [
    ...
    '## Scope',
    '',
    '- Deployed the generated `1.1.2-korte-check` page/data plus shared runtime files updated by the platform workflow into the Book 1 lesson repo.',
    '- Confirmed the route uses `header.ge-topbar`, `main.ge-page[data-golden-ticket-root]`, `golden-ticket-layout.css`, `shared/exit-ticket/1.1.2-korte-check.js`, and `golden-ticket-layout.js`.',
    '- Confirmed the advisory choice route does not load `golden-ticket-graph.js` or legacy task-shell/exit-ticket assets.',
    '- Confirmed `hintsAbsent: true`; no hint-state screenshot is required for this route.',
    '',
    '## Rendered Evidence',
    '',
    ...
    '- No paragraph completion or completion-language claim.',
    '- No diagnostics, mastery, automatic sequencing, grading, summative use, or PV.',
    '- No Scale Gate 1.',
  ...
  try {
    ...
    const proofJson = {
      ...
      static_contract: {
        ...
        advisory_authority_passed: true,
      },
      rendered_states: {
        required_cases: cases.map((item) => item.case),
        captured_cases: captured.map((item) => item.case),
        desktop_mobile_dark_captured: true,
        wrong_retry_captured: captured.some((item) => item.case === 'desktop-light-wrong-retry' && item.proof.firstTaskRetry === true),
        local_success_captured: captured.some((item) => item.case === 'desktop-light-local-success' && item.proof.firstTaskGood === true),
        completed_advisory_captured: captured.some((item) => item.case === 'desktop-light-completed-advisory' && item.proof.completionVisible === true),
        reload_captured: captured.some((item) => item.case === 'route-reload' && item.proof.sourceKey === '1.1.2-korte-check'),
        hintsAbsent: true,
      },
      authority: {
        advisory_only: true,
        generated_lesson_output_changed: true,
        migrated_routes: ['1.1.2-korte-check'],
        broad_rollout_authorized: false,
        exit_ticket_authority_changed: false,
        target_equivalent_proof_authorized: false,
        paragraph_completion_authorized: false,
