...
Roadmap version: `v2.97-mtu-h2f-conditional-cli-execution`
...
|--------|------|-----------|---------------|
| MTU-H2G | A20 Split And Affected-Mapping Packet | no | **ACTIVE OPERATIONAL NEXT ACTION.** Prepare a non-mutating planning packet for the held `A20` lane after MTU-H2F. Audit active `A20` usages, especially target exercise `4.1.2`, classify each use as derived-MK, given-MK, generic/ambiguous, stale, or generator-impact-only, and decide whether `A20` should be kept, updated, split, deprecated, or replaced while `A91` carries the given-MK route. Include `GEN_A20` impact review, affected-mapping route, exact later CLI/rollback expectations, and review questions. No direct `A20` mutation, hand edits to `references/machine/` or `references/external/`, candidate writes, lesson-output mutation, target-exercise promotion, CP-6/Year-1 closure, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or student/product use are authorized. |
...
Next action: run `MTU-H2F Conditional Solo q1-q3 CLI Execution`. GATE-MTU-H2E closed as `pass_with_conditions` after the H2E packet and cited evidence were pushed to remote commit `52ffc484b270182964283e20cd696aca6ce5f9e6`. MTU-H2F may execute only the reviewed reference-CLI command set for `A12`, `A88`, `A89`, `A90`, `A92`, and `A93`; `A20` remains out of scope except as a held lane requiring a separate split/deprecate/replacement and affected-mapping packet. Before execution, run final preflight: clean-worktree or explained status, fresh ID absence for `A88`/`A89`/`A90`/`A92`/`A93`, presence checks for `A12`/`A20`, extracted-spec logging and comparison, `A12 --dry-run` with `A2.11` retained, and proof that no `A20` command exists. After execution, refresh generator-readiness records and prove missing `GEN_A88`, `GEN_A89`, `GEN_A90`, `GEN_A92`, and `GEN_A93` are generator-blocked/not-yet-interactive or otherwise not exposed to student-facing skill-tree/PV routes. No hand edits to `references/machine/` or `references/external/`, candidate writes, lesson-output mutation, target-exercise promotion, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or student/product use are authorized. GAME-UX-2 has already supplied the bounded checkpoint engine support; lesson-side broad companion scaling remains blocked by the lesson review/Scale Gate path, while the reference-side active lane is now MTU-H2F.
