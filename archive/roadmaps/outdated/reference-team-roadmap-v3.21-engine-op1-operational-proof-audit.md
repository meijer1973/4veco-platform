...
Roadmap version: `v3.21-engine-op1-operational-proof-audit`
...
Gate 1, or student/product use. GAME-UX-3A has since closed; top operational
...
runtime is not yet visible in generated Book output; top operational next
action is `SKILLMAP-OP-1`.
...
is `SKILLMAP-OP-1`; no Scale Gate 1, target-equivalent completion claim,
diagnostics, adaptive routing, mastery/sequencing, student-facing AI,
...
- `LESSON-SCALE-1` tracks the lesson-side pre-scale dependency: L1.7A rejected
  ...
  supplied live-route audit evidence. Broad companion/product scaling must
  still wait for SKILLMAP-OP-1, GRAPH-UX-2, MATH-UX-2, REASON-UX-2,
  GAME-ARCH-1, GATE-ENGINE-1, L1.7B-Q2, GATE-L1.7B-Q2, REV-STD-1, minimal
  curriculum-versioning readiness, and Scale Gate 1.
...
Next action: start `SKILLMAP-OP-1 Student-Visible Skill-Map Route`. ENGINE-OP-1 has now audited what students actually see and do across the shared skill-map route, math game, graph game, reasoning game, and checkpoint route for `1.1.1`, `1.1.2`, and `1.1.3`. The audit found that graph practice for `1.1.3` and math practice for `1.1.2` are operational practice routes, but the shared route layer is not yet product-coherent: `1.1.2` reasoning/graph route panels are empty, `1.1.1` reasoning is mis-scoped toward unrelated calculation/graph skills, and generated output does not yet use the GAME-UX-3A shared task shell. SKILLMAP-OP-1 should now make the shared skill-map visibly useful and scoped for each practice mode before graph/math/reasoning integration, target-equivalent checkpoint work, GATE-ENGINE-1, or Scale Gate 1. It must not claim target-equivalent proof, write target-exercise fields, create candidate storage, authorize diagnostics/adaptive/mastery/sequencing/student-facing AI/summative/PV/product use, or advance Scale Gate 1.
