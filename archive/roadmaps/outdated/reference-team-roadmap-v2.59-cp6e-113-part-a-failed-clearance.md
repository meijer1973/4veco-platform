...
Roadmap version: `v2.59-cp6e-113-part-a-failed-clearance`
...
|--------|------|-----------|---------------|
| CP.6f | 1.1.3 Part A Remediation Recheck | no | **ACTIVE / BLOCKED ON LESSON-SIDE REMEDIATION AFTER CP.6e.** CP.6e failed to clear the `1.1.3` Part A flag because the live paragraph still introduces figures in the order `1 -> 3 -> 2` rather than `1 -> 2 -> 3`. Route lesson-side remediation/regeneration for figure numbering, then run a focused references-side recheck before any unconditioned CP-6 closure proposal. No lesson-output hand patching, protected mutation, target-exercise promotion, CP-6 closure, or Year-1 closure is authorized. |
| EX-0 | Exam Ingestion Contract Design | no | FUTURE DESIGN SPRINT after CP.6f recheck or an explicit human-approved hold on the remaining `1.1.3` Part A blocker. Define overlay-first schema and review procedure for official exam question, source annex, official answer model, scoring rules, MTU decomposition, graph/source operations, and answer-writing operations. No protected mutation, no external-source mutation, no unit minting, and no student/product authorization. |
...
| Sprint | Name | Completed | Current State |
|--------|------|-----------|---------------|
| CP.6e | Focused 1.1.3 Part A Re-Review | yes | Completed as a non-mutating focused Part A re-review with `failed_clearance`. The current live `1.1.3 Grafieken en tabellen – paragraaf.md` still introduces figures in first-use order `1 -> 3 -> 2` instead of `1 -> 2 -> 3`, so the remaining Part A figure-numbering flag is not cleared. The repeated worked example in `opgaven.md` is recorded as accepted standalone-exercise scaffolding, not the remaining hard blocker. Output: `references/data/sprints/CP.6e-113-part-a-rereview.json`, `reports/reference-planning/CP.6e-113-part-a-rereview.md`, and `reports/reference-planning/CP.6e-113-part-a-remediation-handoff.md`. No protected mutation, lesson-output mutation, lesson review/quality-ref hand patching, target-exercise promotion, placeholder finalization, unit minting, CP-6 closure, or Year-1 closure was authorized. CP-6 remains blocked pending lesson-side remediation/regeneration and references-side recheck. |
| CP.6d | Book 1 Graph-Heavy Evidence Upgrade | yes | Completed as a non-mutating graph-heavy evidence ledger upgrade. Checked the live post-L-CP6A lesson repo, not stale REF-CT2 paths: active `1.3.2` is `Marktevenwicht`, active `1.3.3` is `Verschuivingen en nieuw evenwicht`, and active Chapter 1.3 has 0 current source/lesson mismatches. Recorded 9 graph-heavy active-v5 Book 1 records, 9 current Part A review files, 3 companion-review-required records with current companion visual reviews, 3 `schema_version: 2` quality refs, 6 legacy/pre-schema quality refs needing later quality workflow, 1 open `1.1.3` Part A `FLAG`, and 0 records allowed as CP-6 closure evidence now. Output: `references/data/sprints/CP.6d-graph-heavy-evidence.json` and `reports/reference-planning/CP.6d-graph-heavy-evidence.md`. No protected mutation, lesson-output mutation, lesson quality-ref hand patching, companion review fabrication, target-exercise promotion, placeholder finalization, unit minting, CP-6 closure, or Year-1 closure was authorized. CP.6e has now failed clearance, so CP-6 remains blocked pending lesson-side remediation and CP.6f recheck. |
| CP.6c | Year-1 MTU Backfill Classification | yes | Completed as a non-mutating MTU backfill classification sprint. Classified the nine REF-CT1 Year-1 candidates against the current live registry: existing-unit mappings to `A45`, `A46`, `A47`, `A48`, `A49`, and `A51`; one merge/design candidate for normal/inferior goods around successor units `A17`/`D11`/`D33` with deprecated `D04` retained only as historical context; one deferred kink-in-collective-demand candidate; and one true missing simultaneous-shift reasoning operation for later governed review. Output: `references/data/sprints/CP.6c-mtu-backfill-classification.json` and `reports/reference-planning/CP.6c-mtu-backfill-classification.md`. No protected mutation, unit minting, target-exercise promotion, placeholder finalization, CP-6 closure, or Year-1 closure was authorized. CP.6d is complete; CP.6e failed clearance; CP.6f recheck remains required after lesson-side remediation. |
| CP.6b | Year-1 Target-Exercise Review | yes | Completed as a non-mutating target-exercise review/design sprint. Recorded 12 active-v5 Book 1 target-exercise records, 9 migrated records needing explicit v5 review artifacts, 3 placeholder records with draft gemengde-opgaven integration designs, 0 reviewed-final records, 0 target-exercise promotions, and 0 placeholder finalizations. Output: `references/data/sprints/CP.6b-target-exercise-review.json` and `reports/reference-planning/CP.6b-target-exercise-review.md`. CP-6 and Year 1 remain open; CP.6d is complete, CP.6e failed clearance, and CP.6f recheck remains required before any responsible closure proposal. |
...
- `CP.6e` completed: focused `1.1.3` Part A re-review failed clearance because figure first-use order remains `1 -> 3 -> 2`.
- `CP.6f` active next: references-side recheck after lesson-side remediation/regeneration evidence for the `1.1.3` Part A figure-numbering issue.
- `EX-0` planned after CP.6f recheck or explicit human-approved hold: exam-ingestion contract design for prompt,
...
CP.6f 1.1.3 Part A remediation recheck
...
Next action: route lesson-side remediation/regeneration for `1.1.3` Part A figure numbering, then run `CP.6f 1.1.3 Part A Remediation Recheck`. CP.6a plus lesson-team L-CP6A cleared the `1.3.2`/`1.3.3` source/lesson mismatch with carried conditions, CP.6b recorded the non-final target-exercise review/design packet, CP.6c classified the nine Year-1 backfill candidates against the current live MTU registry, CP.6d upgraded the graph-heavy evidence ledger against the live post-L-CP6A lesson repo, and CP.6e failed to clear the remaining `1.1.3` Part A flag because the live figure first-use order is still `1 -> 3 -> 2`. CP.6e does not close CP-6 or Year 1: target exercises remain non-final, six graph-heavy quality refs still need later quality workflow before closure reliance, and the simultaneous-shift true missing operation needs later governed review before any mutation. EX-0 through EX-2 remain future strategic lanes after CP.6f recheck or an explicit human-approved hold on the remaining blocker. The remaining blocks on diagnostics, adaptive routing, mastery, sequencing, student-facing AI, summative use, PV projection, and PV machine promotion remain in force.
