...
Last strategic update: 2026-05-29
Roadmap ID: `references-team-roadmap`
Roadmap version: `v3.15-gate-mtu-h4a-pass-with-conditions`
...
|--------|------|-----------|---------------|
| MTU-H4B | Answer-Form Bounded CLI Execution Packet | no | **ACTIVE NEXT SPRINT AFTER GATE-MTU-H4A.** Prepare a non-mutating execution packet for the H4A-accepted answer-form lanes only: `A96` bereken, `A97` uitleg_dat, `A98` uitleg_of, `A99` leg uit met voorbeeld, `A80` noem/geef-aan with split-if-needed condition, and `A81` bron gebruiken only as source-use modifier plus underlying answer form. Quality floor: produce exact `unit-add` commands, print/log every extracted spec before command execution, prove simulated catalog validation before mutation, validate proposed exam codes against the syllabus/eindtermen registry, prove generator implementation or generator-blocked/non-interactive status, prove no student-facing route exposes generator-blocked answer-form units, preserve `A100` rejection and `A71` hold, keep graph/draw/shade, Type 4 motiveer/classificatie, and analysis/evaluation held, and keep q3/q15 EX answer-skill overlays visible with no candidate writes. Review gate: H4B must prepare the later human execution-authorization review packet before any command runs. This row authorizes no protected reference mutation, unit minting, candidate storage creation, candidate writes, target-exercise mutation, generated projection refresh, lesson output, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, Scale Gate 1, or student/product use. |
| LESSON-SCALE-1 | Lesson Companion Pre-Scale Gates Tracking | no | **TRACKING DEPENDENCY, NOT A REFERENCE EXECUTION SPRINT.** Lesson L1.7A closed PASS WITH FLAGS and rejected broad companion scaling. Closed lesson prerequisites now include `L1.7B-C`, `L1.7B-R`, `GATE-L1.7B`, `L1.7C-0`, `L1.7C`, `L1.7D`, `L2.0`, `L1.7C-MATH`, `L1.7B-MAP`, `L1.7B-P23`, and `SPEC-ET-1`. Platform `GAME-UX-1` supplied shared skill-map runtime support; platform `GAME-UX-2` supplied source-controlled `1.1.1` checkpoint support. `L1.7C-MATH` restored scoped skill-tree math as primary `Rekenen`; `L1.7B-MAP` fixed checkpoint metadata and kept `targetReadinessEvidence: false`; `L1.7B-P23` accepted the stop/handoff because choice-only checks cannot honestly cover `1.1.2` calculation/index or `1.1.3` graph/table operation chains; `SPEC-ET-1` corrected the stable specs so exit tickets are target-equivalent proof tasks, not readiness-to-try checks. SYNC-4 makes the shared task-type UI part of the product end-state and adds the operational engine proof track. GATE-MTU-H4A closed PASS WITH CONDITIONS for later bounded execution-packet preparation only; H4B must still prepare non-mutating execution evidence before any command runs. Reference/product planning must not treat the lesson companion pipeline as scale-approved until `MTU-H4B`, `EX-LESSON-1`, `GAME-UX-3A`, `ENGINE-OP-1`, `SKILLMAP-OP-1`, `GRAPH-UX-2`, `MATH-UX-2`, `REASON-UX-2`, `GAME-ARCH-1`, `GATE-ENGINE-1`, lesson `L1.7B-Q2`, `GATE-L1.7B-Q2`, `REV-STD-1`, minimal curriculum-versioning readiness, and Scale Gate 1 resolve or are explicitly waived while preserving blocks on diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, and PV machine promotion. |
...
- `LESSON-SCALE-1` tracks the lesson-side pre-scale dependency: L1.7A rejected
  ...
  companion/product scaling must still wait for MTU-H4B, GAME-UX-3A,
...
Next action: start `MTU-H4B Answer-Form Bounded CLI Execution Packet` with a
checkable sprint plan. H4B may prepare a non-mutating execution packet for
`A80`, `A81`, and `A96`-`A99` only. The plan must define the quality floor,
specification requirements, proof artifacts, validation stack, rollback path,
generator/exposure evidence, exam-code validation, and the later human
execution-authorization gate that will judge whether commands may run. It must
print/log every extracted spec before any later command, prove simulated
catalog validation before mutation, keep `A100` rejected and `A71` held, keep
graph/draw/shade, Type 4 motiveer/classificatie, and analysis/evaluation held,
keep q3/q15 EX overlays visible, and keep target-exercise fields, candidate
storage, candidate writes, projection refresh, lesson output, and product use
out of scope. No hand edits to `references/machine/` or
`references/external`, unit minting, candidate storage creation, candidate
writes, target-exercise mutation, generated projection refresh,
...
Gate 1, or student/product use are authorized. GAME-UX-3A remains the shared
task-type UX foundation dependency, but must not claim live answer-form MTUs or
target-equivalent exit-ticket proof until later governed source mutations and
evidence close.
