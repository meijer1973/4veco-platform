...
Roadmap version: `v3.16-mtu-h4b-execution-packet`
...
|--------|------|-----------|---------------|
| GATE-MTU-H4B | Answer-Form CLI Execution Human Review | no | **ACTIVE NEXT GATE AFTER MTU-H4B.** Review the pushed H4B execution packet and decide whether a later bounded execution sprint may run the exact `unit-add` commands for `A96` bereken, `A97` uitleg_dat, `A98` uitleg_of, `A99` leg uit met voorbeeld, `A80` noem/geef-aan with split-if-needed condition, and `A81` bron gebruiken only as source-use modifier plus underlying answer form. Quality floor: inspect the exact command payloads, simulated catalog validation, exam-code validation, ID allocation (`A100` rejected, `A71` held), generator/exposure guardrails, held graph/Type 4/analysis lanes, q3/q15 EX overlay boundary, no target-exercise field writes, no candidate storage, rollback, validation stack, and remote evidence. This gate itself authorizes no protected reference mutation, unit minting, candidate storage creation, candidate writes, target-exercise mutation, generated projection refresh, lesson output, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, Scale Gate 1, or student/product use. |
| LESSON-SCALE-1 | Lesson Companion Pre-Scale Gates Tracking | no | **TRACKING DEPENDENCY, NOT A REFERENCE EXECUTION SPRINT.** Lesson L1.7A closed PASS WITH FLAGS and rejected broad companion scaling. Closed lesson prerequisites now include `L1.7B-C`, `L1.7B-R`, `GATE-L1.7B`, `L1.7C-0`, `L1.7C`, `L1.7D`, `L2.0`, `L1.7C-MATH`, `L1.7B-MAP`, `L1.7B-P23`, and `SPEC-ET-1`. Platform `GAME-UX-1` supplied shared skill-map runtime support; platform `GAME-UX-2` supplied source-controlled `1.1.1` checkpoint support. `L1.7C-MATH` restored scoped skill-tree math as primary `Rekenen`; `L1.7B-MAP` fixed checkpoint metadata and kept `targetReadinessEvidence: false`; `L1.7B-P23` accepted the stop/handoff because choice-only checks cannot honestly cover `1.1.2` calculation/index or `1.1.3` graph/table operation chains; `SPEC-ET-1` corrected the stable specs so exit tickets are target-equivalent proof tasks, not readiness-to-try checks. SYNC-4 makes the shared task-type UI part of the product end-state and adds the operational engine proof track. GATE-MTU-H4A closed PASS WITH CONDITIONS for later bounded execution-packet preparation only; MTU-H4B prepared the non-mutating execution packet; GATE-MTU-H4B must still close before any command runs. Reference/product planning must not treat the lesson companion pipeline as scale-approved until `GATE-MTU-H4B`, `EX-LESSON-1`, `GAME-UX-3A`, `ENGINE-OP-1`, `SKILLMAP-OP-1`, `GRAPH-UX-2`, `MATH-UX-2`, `REASON-UX-2`, `GAME-ARCH-1`, `GATE-ENGINE-1`, lesson `L1.7B-Q2`, `GATE-L1.7B-Q2`, `REV-STD-1`, minimal curriculum-versioning readiness, and Scale Gate 1 resolve or are explicitly waived while preserving blocks on diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, and PV machine promotion. |
...
- `LESSON-SCALE-1` tracks the lesson-side pre-scale dependency: L1.7A rejected
  ...
  companion/product scaling must still wait for GATE-MTU-H4B, GAME-UX-3A,
...
GATE-MTU-H4B answer-form CLI execution human review
...
Next action: run `GATE-MTU-H4B Answer-Form CLI Execution Human Review` after committing and pushing the H4B packet and cited evidence. The gate must show the full question list, ask calibration questions, ask one question at a time, record answers, analyze patterns, ask targeted follow-ups when authority is ambiguous, draft a closure proposal, and require explicit human confirmation before writing a closure record. The gate may authorize only a later bounded execution sprint for accepted H4B commands, or it may revise/hold the packet. It must not directly execute `unit-add`, create candidate storage, write target-exercise fields, refresh projections, produce lesson output, authorize diagnostics/adaptive/mastery/sequencing/student-facing AI/summative/PV/product use, consume `A71`, treat `A100` as valid, treat `bron` as standalone, or expose generator-blocked answer-form units to students. GAME-UX-3A remains the shared task-type UX foundation dependency, but must not claim live answer-form MTUs or target-equivalent exit-ticket proof until later governed source mutations and evidence close.
