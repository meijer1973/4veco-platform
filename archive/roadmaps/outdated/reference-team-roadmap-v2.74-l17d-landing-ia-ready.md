...
Roadmap version: `v2.74-l17d-landing-ia-ready`
...
- `LESSON-SCALE-1` tracks the lesson-side pre-scale dependency: L1.7A has
  ...
  Broad companion/product scaling must still wait for L1.7D human-review
  closure, L2.0, a resumed
...
Next action: start `EX-5 Operation And Answer-Skill Registry Contract` as a bounded tooling/design sprint only. CP.6a plus lesson-team L-CP6A cleared the `1.3.2`/`1.3.3` source/lesson mismatch with carried conditions, CP.6b recorded the non-final target-exercise review/design packet, CP.6c classified the nine Year-1 backfill candidates against the current live MTU registry, CP.6d upgraded the graph-heavy evidence ledger against the live post-L-CP6A lesson repo, CP.6e failed the first `1.1.3` Part A figure-order recheck, CP.6f cleared that focused figure-numbering blocker after lesson-team L-CP6E, EX-0 defined the exam-ingestion contract, GATE-EX0 closed as `pass_with_conditions`, EX-1 created the three bounded pilot exam-ingestion overlays with q19 graph/source gaps carried as blocking, GATE-EX2 closed as `pass_with_conditions` for classification and routing only, EX-3 reported the reviewed classifications, EX-4 prepared mutation-planning artifacts, GATE-EX4 closed as `pass_with_conditions` for routing/design only, lesson L1.7A rejected broad companion scaling after green current validators, L1.7B recorded the exit-ticket companion contract while pausing MVP implementation behind source-control/product-boundary/shared-skill-map blockers, L1.7C-0 closed the shared skill-map contract, GAME-UX-1 implemented the platform shared skill-map runtime support without lesson-output or product-use authorization, lesson L1.7C closed PASS WITH FLAGS after consuming the shared route in the game row, and lesson L1.7D reached technical QA green / ready for human review after generator-owned landing IA cleanup. EX-5 should define operation-registry storage, answer-skill storage, schemas, validators, q19 source-annex/graph-object extraction contract and validator, rollback, audit evidence, and stop conditions for hidden q19 gaps and hidden q3/q15 answer-skill gaps. CP-6 and Year 1 remain open: target exercises remain non-final, six graph-heavy quality refs still need later quality workflow before closure reliance, and the simultaneous-shift true missing operation needs later governed review before any mutation. Lesson-side broad companion scaling remains blocked until L1.7D human-review closure, L2.0, a resumed boundary-safe exit-ticket MVP, and Scale Gate 1 establish landing-page IA, house-style policy, checkpoint review evidence, and remaining game-row scaling quality. The remaining blocks on protected mutation, external-source mutation, machine-reference mutation, unit minting, operation-registry mutation, answer-skill mutation, PV/graph mutation, lesson-output mutation, diagnostics, adaptive routing, mastery, sequencing, student-facing AI, summative use, PV projection, and PV machine promotion remain in force.
