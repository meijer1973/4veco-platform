...
Roadmap version: `v3.18-mtu-h4c-answer-form-executed`
...
- `LESSON-SCALE-1` tracks the lesson-side pre-scale dependency: L1.7A rejected
  ...
  has executed the bounded answer-form unit lane through CLI-only mutation. Broad
  companion/product scaling must still wait for EX-LESSON-1, GAME-UX-3A,
...
EX-LESSON-1 exam-ingestion end-state integration
...
Next action: start `EX-LESSON-1 Exam-Ingestion End-State Integration`. MTU-H4C has executed the bounded answer-form unit lane and produced no lesson output or product exposure. EX-LESSON-1 should update the lesson-roadmap gates, paragraph-plan requirements, and later lesson-build handoffs so official CvTE and CvTE-derived target exercises trace prompt, source annexes, figures/tables/graphs, correction model, point allocation, answer-construction requirements, concepts, calculations, graph/table/source operations, reasoning operations, and answer-writing requirements into the explanation, skill-map route, shared task shell, target-equivalent exit ticket, and answer model. EX-LESSON-1 must not generate lesson output, mutate protected references, write target-exercise fields, create candidate storage, authorize diagnostics/adaptive/mastery/sequencing/student-facing AI/summative/PV/product use, or advance Scale Gate 1. GAME-UX-3A remains the shared task-type UX foundation dependency and should consume the EX-LESSON-1 specification boundary once it is logged.
