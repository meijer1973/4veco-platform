...
Roadmap version: `v2.60-cp6f-113-part-a-cleared`
...
|--------|------|-----------|---------------|
| EX-0 | Exam Ingestion Contract Design | no | **ACTIVE / DESIGN SPRINT.** CP.6f cleared the focused `1.1.3` Part A figure-numbering blocker, so the next strategic lane is to define the overlay-first schema and review procedure for official exam question, source annex, official answer model, scoring rules, MTU decomposition, graph/source operations, and answer-writing operations. No protected mutation, no external-source mutation, no unit minting, no CP-6 closure, no Year-1 closure, and no student/product authorization. |
...
- `EX-0` active next: exam-ingestion contract design for prompt,
...
EX-0 Exam ingestion contract design active
...
Next action: start `EX-0 Exam Ingestion Contract Design`. CP.6a plus lesson-team L-CP6A cleared the `1.3.2`/`1.3.3` source/lesson mismatch with carried conditions, CP.6b recorded the non-final target-exercise review/design packet, CP.6c classified the nine Year-1 backfill candidates against the current live MTU registry, CP.6d upgraded the graph-heavy evidence ledger against the live post-L-CP6A lesson repo, CP.6e failed the first `1.1.3` Part A figure-order recheck, and CP.6f has now cleared that focused figure-numbering blocker after lesson-team L-CP6E. CP.6f does not close CP-6 or Year 1: target exercises remain non-final, six graph-heavy quality refs still need later quality workflow before closure reliance, and the simultaneous-shift true missing operation needs later governed review before any mutation. EX-0 through EX-2 are the next bounded strategic lanes for exam-ingestion contract/pilot/review design. The remaining blocks on diagnostics, adaptive routing, mastery, sequencing, student-facing AI, summative use, PV projection, and PV machine promotion remain in force.
