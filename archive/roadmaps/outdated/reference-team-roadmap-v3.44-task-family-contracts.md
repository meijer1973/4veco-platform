...
Roadmap version: `v3.44-task-family-contracts`
...
| TASK-FAMILY-CLOZE-TILE-1 | Cloze Tile-Select Task-Family Implementation | no | Add `cloze_tile_select` for index-points-versus-percent, source-label, formula-fill, graph-reading conclusion, and reasoning-sentence completion tasks. This is the first construction implementation because it exposes misconceptions with low typing friction. |
...
| TASK-FAMILY-CLOZE-TILE-1 | Cloze Tile-Select Task-Family Implementation | no | Implement selectable-tile cloze tasks as the first construction family. Priority use cases: index-points versus percent, source labels, graph-reading conclusions, and compact reasoning completions where distractors expose misconceptions. |
...
Next action: proceed to `TASK-FAMILY-CLOZE-TILE-1` as the next Product Proof
Track implementation sprint, using the closed `TASK-FAMILY-CONSTRUCT-1`
contract. `TASK-FAMILY-SENTENCE-1` and `TASK-FAMILY-FORMULA-1` remain the next
highest-priority construction implementation sprints after that. `GAME-ROUTE-AFFORDANCE-1`
...
structured-choice/construction contract sprints.
