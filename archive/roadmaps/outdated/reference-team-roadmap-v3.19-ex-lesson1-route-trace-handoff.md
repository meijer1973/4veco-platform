...
Roadmap version: `v3.19-ex-lesson1-route-trace-handoff`
...
Gate 1, or student/product use. Top operational next action is `GAME-UX-3A`.
...
- `LESSON-SCALE-1` tracks the lesson-side pre-scale dependency: L1.7A rejected
  ...
  EX-LESSON-1 has supplied the exam-target route-trace handoff. Broad
  companion/product scaling must still wait for GAME-UX-3A, ENGINE-OP-1,
...
Next action: start `GAME-UX-3A Shared Task-Type UX Foundation`. MTU-H4C has executed the bounded answer-form unit lane, and EX-LESSON-1 has converted the exam-ingestion end state into paragraph-plan, authoring-guide, review-prompt, roadmap, and checklist requirements without generated lesson output or product exposure. GAME-UX-3A should now build the reusable task shell for target-equivalent exit tickets, checkpoint-only local checks, graph/table practice, math/calculation practice, and exam-style answer-form requirements. It may consume H4/H4A/H4B/H4C answer-form decisions and the EX-LESSON-1 route-trace checklist as design input, but it must not expose generator-blocked answer-form units, claim target-equivalent proof, write target-exercise fields, create candidate storage, authorize diagnostics/adaptive/mastery/sequencing/student-facing AI/summative/PV/product use, or advance Scale Gate 1.
