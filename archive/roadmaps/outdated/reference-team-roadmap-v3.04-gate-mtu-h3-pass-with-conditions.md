...
Roadmap version: `v3.04-gate-mtu-h3-pass-with-conditions`
...
|--------|------|-----------|---------------|
| MTU-H3A | Incidence Pass-Through CLI-Mutation Planning Packet | no | **ACTIVE OPERATIONAL NEXT ACTION.** Prepare a later bounded CLI-mutation planning packet for the GATE-MTU-H3 accepted routing only: narrow `D07` to tax afwentelingspercentage/percentage burden, plan `D41`/equivalent for tax wedge and Pc/Pp graphical labeling, `D42`/equivalent for euro tax burden amounts, `D43`/`D44` or equivalents for subsidy effective prices and benefit-sharing, `D45`/equivalent for relative elasticity explanation, and `D46`/equivalent for true cost-shock pass-through share while keeping `A93` bounded to percentage price change. The planning packet must include exact proposed specs, exact mapping proposal for `3.1.1`/`3.1.2`/`3.1.3`, dependency audit, A93 boundary proof, rollback, validation, and projection/source boundaries. No `D07` mutation, successor-unit minting, target-exercise mutation, candidate writes, generated projection refresh, PV projection, PV machine promotion, lesson output, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, or student/product use is authorized. |
