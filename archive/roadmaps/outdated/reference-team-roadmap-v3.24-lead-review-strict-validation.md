...
Roadmap version: `v3.24-lead-review-strict-validation`
...
Gate 1, or student/product use. GAME-UX-3A, ENGINE-OP-1, and SKILLMAP-OP-1
have since closed; top operational next action is `GRAPH-UX-2`.
...
projection refresh, diagnostics, adaptive routing, mastery/sequencing,
student-facing AI, summative use, PV projection, PV machine promotion, Scale
Gate 1, or student/product use is authorized. ENGINE-OP-1 has since shown the
runtime is not yet visible in generated Book output; SKILLMAP-OP-1 has since
made the route layer visible and scoped. Top operational next action is
`GRAPH-UX-2`.
...
Top operational next action is `GRAPH-UX-2`; no Scale Gate 1,
...
- `LESSON-SCALE-1` tracks the lesson-side pre-scale dependency: L1.7A rejected
  ...
  visible-route proof for Book 1 `1.1.1`, `1.1.2`, and `1.1.3`. Broad
  companion/product scaling must still wait for GRAPH-UX-2, MATH-UX-2,
  REASON-UX-2, GAME-ARCH-1, GATE-ENGINE-1, L1.7B-Q2, GATE-L1.7B-Q2,
...
Next action: start `GRAPH-UX-2 Graph Game + Checkpoint UI Integration`. SKILLMAP-OP-1 has now made the shared route visible and scoped in live Book 1 output for `1.1.1`, `1.1.2`, and `1.1.3`, with paragraph target, recommended focus, local progress, and practice action shown without internal MTU codes. GRAPH-UX-2 should now use the GAME-UX-3A task shell to upgrade graph/table practice and checkpoint-style graph tasks for `1.1.3`, including table-value selection, graph reading, economic axis convention, interpolation, point placement or graph-construction substitute, and less-labelled variants. It must not claim target-equivalent proof, write target-exercise fields, create candidate storage, authorize diagnostics/adaptive/mastery/sequencing/student-facing AI/summative/PV/product use, or advance Scale Gate 1.
