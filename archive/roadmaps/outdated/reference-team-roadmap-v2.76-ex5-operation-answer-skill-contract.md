...
Last strategic update: 2026-05-24
Roadmap ID: `references-team-roadmap`
Roadmap version: `v2.76-ex5-operation-answer-skill-contract`
...
| LESSON-SCALE-1 | Lesson Companion Pre-Scale Gates Tracking | no | **TRACKING DEPENDENCY, NOT A REFERENCE EXECUTION SPRINT.** Lesson L1.7A closed PASS WITH FLAGS and rejected broad companion scaling. Lesson L1.7B recorded the exit-ticket companion-completion contract and paused MVP implementation behind source-control/product-boundary/shared-skill-map blockers. Lesson L1.7C-0 then closed the shared skill-map engine contract, and platform `GAME-UX-1` implemented the shared skill-map runtime support: three separate practice engines can now request one shared route/progression layer with aspect filters, compact/route/restricted-full modes, and non-mastery progress language. Lesson L1.7C first human review returned REVISE because the route panel could expose internal skill IDs; the targeted platform/lesson fix now renders student-facing labels, focused human recheck accepted the fix, and L1.7C closed PASS WITH FLAGS. Lesson L1.7D has now closed PASS WITH FLAGS after generator-owned landing IA cleanup; landing pages use `Start`, `Leer`, `Oefen`, optional `Check`, and `Verdiep`, with `Check` hidden until reviewed exit-ticket output exists. Lesson L2.0 is now active as a foundation-hardening cleanup sprint; a resumed boundary-safe exit-ticket MVP and Scale Gate 1 remain after it. Reference/product planning must not treat the lesson companion pipeline as scale-approved until those lesson-side gates exist and preserve blocks on diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, and PV machine promotion. |
...
| Product Gate | Re-Evaluate R10-R13 Product Surfaces | no | Blocked. Diagnostics, adaptive routing, student-facing AI, teacher cockpit deployment, automatic sequencing, mastery, and summative uses remain out of scope until data-quality gates justify them. Lesson L1.7A explicitly rejected broad companion scaling; L1.7B recorded the companion-set contract but paused exit-ticket MVP implementation; L1.7C-0 closed the shared skill-map contract; platform `GAME-UX-1` implemented the shared route/view-model support without authorizing product use; lesson L1.7C closed PASS WITH FLAGS after consuming that shared route in the game row; and lesson L1.7D closed PASS WITH FLAGS after generator-owned landing IA cleanup. Broad lesson-companion scaling remains blocked until L2.0 closes, the resumed boundary-safe exit-ticket MVP has checkpoint review evidence, and Scale Gate 1 establishes remaining house-style and game-row scaling quality. |
...
Update 2026-05-22: Lesson roadmap gained three explicit pre-scale companion gates after L1.7A: `L1.7B Exit Ticket Game MVP + Companion Completion Contract`, `L1.7C Three-Aspect Game Quality Upgrade`, and `L1.7D Paragraph Landing Page Information Architecture Cleanup`. This reference roadmap now tracks that dependency as `LESSON-SCALE-1`: broad companion/product scaling must wait for the companion-set contract, game-row quality rubric, landing-page IA, and L2.0 house-style burn-down. A local untracked platform candidate exists at `knowledge/exit-ticket-game-1.1.1.zip`; lesson L1.7B must resolve whether that prototype becomes source-controlled review input or remains a source-control gap. This update authorizes no lesson-output mutation, protected reference mutation, target-exercise promotion, placeholder finalization, unit minting, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or product-use claim.
...
Update 2026-05-23: Lesson L1.7A closed PASS WITH FLAGS as a readiness decision sprint. Current lesson validation is green for `1.1.1` through `1.1.3`, procedure contracts, Book 1 health, print scope, and v5 target-exercise counts, but the decision explicitly rejects broad companion scaling. The allowed lesson path remains foundation hardening: L1.7B exit ticket and companion contract, L1.7C-0 shared skill-map contract, L1.7C game-row quality upgrade, L1.7D landing-page IA, L2.0 house-style cleanup, then Scale Gate 1. This update authorizes no lesson-output mutation, protected reference mutation, target-exercise promotion, placeholder finalization, unit minting, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or product-use claim.

Update 2026-05-23: Lesson L1.7B completed a contract-and-stop pass, not a student-output build. The lesson team inspected local untracked prototype `knowledge/exit-ticket-game-1.1.1.zip`, ran its prototype unit test successfully, and recorded it as design evidence only. The companion-completion contract and future exit-ticket MVP scope now exist in `archive/sprints/L1.7B/`, but implementation is paused because the prototype is not source-controlled, uses mastery/score/pass/evidence/adaptive-focus semantics, and depends on compact checkpoint-mode skill-map behavior that L1.7C-0/GAME-UX-1 must define first. Lesson L1.7C-0 is now the active lesson dependency. This update authorizes no lesson-output mutation, prototype import, target-exercise promotion, protected reference mutation, placeholder finalization, unit minting, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or product-use claim.
...
- `LESSON-SCALE-1` tracks the lesson-side pre-scale dependency: L1.7A has
  rejected broad scaling, L1.7B recorded the companion contract and paused
  implementation, L1.7C-0 closed the shared skill-map contract, platform
  `GAME-UX-1` implemented the shared skill-map runtime support, and lesson
  L1.7C closed PASS WITH FLAGS after consuming the shared route in the game row.
  Broad companion/product scaling must still wait for L2.0, a resumed
  boundary-safe exit-ticket MVP, and Scale Gate 1.
...
LESSON-SCALE-1 lesson companion pre-scale gates tracked: L1.7A closed with broad scaling rejected; L1.7B recorded the exit-ticket companion contract and paused MVP implementation; L1.7C-0 closed the shared skill-map contract; GAME-UX-1 implemented platform shared skill-map runtime support; L1.7C closed PASS WITH FLAGS after focused recheck; L1.7D closed PASS WITH FLAGS; L2.0 and a resumed boundary-safe exit-ticket MVP remain before broad companion/product scaling
