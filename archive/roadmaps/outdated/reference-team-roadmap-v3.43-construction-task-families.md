...
Roadmap version: `v3.43-construction-task-families`
...
| TASK-FAMILY-CONSTRUCT-1 | Constrained Construction Task-Family Contract | no | Define the shared construction layer for `cloze_tile_select`, `sentence_builder`, `formula_builder`, `source_value_selection`, `source_chain_builder`, and `label_placement`, including token/tile banks, distractors, ordering/placement rules, partial-feedback limits, and target-operation use cases. No implementation or generated output. |
...
| TASK-FAMILY-CONSTRUCT-1 | Constrained Construction Task-Family Contract | no | Planning sprint for construction-style shared tasks: `cloze_tile_select`, `sentence_builder`, `formula_builder`, `source_value_selection`, `source_chain_builder`, and `label_placement`. It must define response shapes, tile/token-bank rules, distractor policy, ordering/placement semantics, partial-feedback limits, accessibility expectations, and proof-use boundaries before implementation. |
...
Next action: proceed to `TASK-FAMILY-CHOICE-1` and
`TASK-FAMILY-CONSTRUCT-1` as the next low-risk Product Proof Track planning
sprints. They should convert the multiple-choice and constrained-construction
reports into reviewed shared-shell contracts before implementation begins.
`TASK-FAMILY-CLOZE-TILE-1`, `TASK-FAMILY-SENTENCE-1`, and
`TASK-FAMILY-FORMULA-1` are the highest-priority implementation sprints after
the contracts. `GAME-ROUTE-AFFORDANCE-1` remains the next route-affordance
implementation lane and may run after or alongside the task-family contracts
if capacity allows, but `REASON-STD-1`, `CHECK-SHORT-EXIT-2`,
...
structured-choice/construction roadmap insertions.
