...
Roadmap version: `v2.92-mtu-h2c-reduced-scope-cli-execution`
...
|--------|------|-----------|---------------|
| MTU-H2D | Solo q1-q3 Held And Conditional Lane Resolution | no | **ACTIVE OPERATIONAL NEXT ACTION.** Prepare a focused resolution packet before any further Solo q1-q3 MTU execution. Resolve `A12` by retaining `A2.11` in any derivative-MO update spec or routing a later gate that explicitly authorizes removal. Resolve `A20` through a split/deprecate/replacement and affected-mapping plan because target exercise `4.1.2` uses current `A20` in a given-MK context. Decide or revise conditional lanes `A88`, `A89`, `A90`, and `A93`; resolve `A92` by accepting/adding `A89`, revising `A92`, or keeping `A92` held. This is planning/review work only unless a later gate names exact CLI commands, specs, rollback, audit log, validation evidence, and execution authority. No hand edits to `references/machine/` or `references/external/`, candidate writes, lesson-output mutation, CP-6/Year-1 closure, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or student-facing output are authorized. |
...
Next action: run `MTU-H2D Solo q1-q3 Held And Conditional Lane Resolution`. MTU-H2C has already minted the reduced clean set `F19`, `F20`, `A85`, `A86`, `A87`, and `A91` through `unit-add.js`. Do not execute another Solo q1-q3 mutation until H2D resolves the remaining lanes. `A12` must retain `A2.11` in any derivative-MO update spec or receive a later explicit gate authorization for removal. `A20` needs a split/deprecate/replacement and affected-mapping plan because target exercise `4.1.2` uses current `A20` in a given-MK context. `A88`, `A89`, `A90`, and `A93` remain conditional or revise-first. `A92` requires an explicit dependency decision because it depends on conditional `A89`. H2D is planning/review work only unless a later gate names exact CLI commands, specs, rollback, audit log, validation evidence, and execution authority. GAME-UX-2 has already supplied the bounded checkpoint engine support; lesson-side broad companion scaling remains blocked by the lesson review/Scale Gate path, while the reference-side active lane is now MTU-H2D.
