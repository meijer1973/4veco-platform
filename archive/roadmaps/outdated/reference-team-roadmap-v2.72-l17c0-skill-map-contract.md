...
Last strategic update: 2026-05-23
Roadmap ID: `references-team-roadmap`
Roadmap version: `v2.72-l17c0-skill-map-contract`
...
| LESSON-SCALE-1 | Lesson Companion Pre-Scale Gates Tracking | no | **TRACKING DEPENDENCY, NOT A REFERENCE EXECUTION SPRINT.** Lesson L1.7A closed PASS WITH FLAGS and rejected broad companion scaling. Lesson L1.7B recorded the exit-ticket companion-completion contract and paused MVP implementation behind source-control/product-boundary/shared-skill-map blockers. Lesson L1.7C-0 then closed the shared skill-map engine contract: three separate practice engines consume one shared route/progression layer with aspect filters, compact/route/restricted-full modes, and non-mastery progress language. Lesson L1.7C is now active for game-row quality review, followed by L1.7D, L2.0, a resumed boundary-safe exit-ticket MVP when ready, and Scale Gate 1. Reference/product planning must not treat the lesson companion pipeline as scale-approved until those lesson-side gates exist and preserve blocks on diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, and PV machine promotion. |
| GAME-UX-1 | Shared Skill-Map Engine Architecture | no | **PLATFORM SUPPORT FOR CLOSED LESSON CONTRACT L1.7C-0.** Lesson L1.7C-0 accepted the shared skill-map contract and handed implementation to this platform lane. Build or refactor one shared skill-map engine consumed by the reasoning, calculation, and graphical practice engines, plus compact checkpoint mode for the later exit-ticket surface and compact route previews for landing pages. It must expose MTU aspects through skill data, aspect-filtered skill subsets, recommended-next-skill logic, non-mastery state/progress labels, prerequisite path rendering, compact/route/restricted-full display modes, and progress/stars display. It must avoid unfiltered all-skill views by default and must not authorize adaptive diagnostics, mastery, automatic sequencing, student-facing AI, summative use, PV projection, or PV machine promotion. |
...
| Product Gate | Re-Evaluate R10-R13 Product Surfaces | no | Blocked. Diagnostics, adaptive routing, student-facing AI, teacher cockpit deployment, automatic sequencing, mastery, and summative uses remain out of scope until data-quality gates justify them. Lesson L1.7A explicitly rejected broad companion scaling; L1.7B recorded the companion-set contract but paused exit-ticket MVP implementation; L1.7C-0 closed the shared skill-map contract while leaving platform implementation to `GAME-UX-1`. Broad lesson-companion scaling remains blocked until L1.7C, L1.7D, L2.0, the resumed boundary-safe exit-ticket MVP, `GAME-UX-1`, and Scale Gate 1 establish real shared skill-map behavior, game-row quality rubric, landing-page information architecture, and checkpoint review evidence. |
...
- `LESSON-SCALE-1` and `GAME-UX-1` track the lesson-side pre-scale dependency:
  L1.7A has rejected broad scaling, L1.7B recorded the companion contract and
  paused implementation, and L1.7C-0 closed the shared skill-map contract. Broad
  companion/product scaling must still wait for `GAME-UX-1`, L1.7C, L1.7D,
  L2.0, a resumed boundary-safe exit-ticket MVP, and Scale Gate 1.
...
LESSON-SCALE-1 lesson companion pre-scale gates tracked: L1.7A closed with broad scaling rejected; L1.7B recorded the exit-ticket companion contract and paused MVP implementation; L1.7C-0 closed the shared skill-map contract; L1.7C, L1.7D, L2.0, GAME-UX-1 implementation, and a resumed boundary-safe exit-ticket MVP remain before broad companion/product scaling
GAME-UX-1 shared skill-map engine architecture implementation for the closed lesson L1.7C-0 contract
