...
Roadmap version: `v2.91-gate-mtu-h2b-partial-pass`
...
|--------|------|-----------|---------------|
| MTU-H2C | Reduced-Scope Solo q1-q3 CLI Execution Preflight | no | **ACTIVE OPERATIONAL NEXT ACTION.** Proceed only with the reduced-scope execution preflight authorized by GATE-MTU-H2B. Clean lanes after final preflight are `F19`, `F20`, `A85`, `A86`, `A87`, and `A91`. `A92` cannot execute until its dependency on conditional `A89` is resolved by including/accepting `A89`, revising `A92`, or holding `A92`. `A88`, `A89`, `A90`, and `A93` are conditional or revise-first lanes. `A12` is held until an update spec retains `A2.11` or a later gate explicitly justifies removal. `A20` is held because target exercise `4.1.2` uses `A20` in a given-MK context. Any execution must echo extracted JSON specs before commands, prove the reviewed plan file is unchanged or revalidated, run final ID-collision checks, and prove no unintended lesson/candidate/external-source diff. No hand edits to `references/machine/` or `references/external/`, candidate writes, lesson-output mutation, CP-6/Year-1 closure, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or student-facing output are authorized. |
...
MTU-H2C Reduced-scope Solo q1-q3 CLI execution preflight
...
Next action: run `MTU-H2C Reduced-Scope Solo q1-q3 CLI Execution Preflight` under the GATE-MTU-H2B partial-pass conditions. Do not execute H2B as-is. The clean reduced set is `F19`, `F20`, `A85`, `A86`, `A87`, and `A91` after final preflight. `A92` requires an explicit dependency decision because it depends on conditional `A89`. `A88`, `A89`, `A90`, and `A93` are conditional or revise-first lanes. `A12` remains held until its update spec retains `A2.11` or a later gate explicitly justifies removal, and `A20` remains held because target exercise `4.1.2` uses `A20` in a given-MK context. Before any execution, echo each extracted JSON spec, prove the reviewed plan file is unchanged or revalidated, rerun collision checks, and preserve no-unintended lesson-output, candidate-storage, `references/external/`, or product-use diff. GAME-UX-2 has already supplied the bounded checkpoint engine support; lesson-side broad companion scaling remains blocked by the lesson review/Scale Gate path, while the reference-side active lane is now MTU-H2C.
