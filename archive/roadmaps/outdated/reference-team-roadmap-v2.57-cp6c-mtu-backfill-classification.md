...
Roadmap version: `v2.57-cp6c-mtu-backfill-classification`
...
|--------|------|-----------|---------------|
| CP.6d | Book 1 Graph-Heavy Evidence Upgrade | no | **ACTIVE / REMEDIATION LANE 4 AFTER CP.6c.** CP.6c classified the nine Year-1 MTU backfill candidates against the current live registry: six existing-unit mappings, one merge/design candidate around the retired D04 successor cluster, one defer candidate, and one true missing simultaneous-shift operation for later review. Upgrade current Part A evidence for graph-heavy records, and Part B evidence where companion material exists, is used as evidence, or is in sprint scope. Legacy quality-ref alone is not enough for CP-6 closure. |
...
| Sprint | Name | Completed | Current State |
|--------|------|-----------|---------------|
| CP.6c | Year-1 MTU Backfill Classification | yes | Completed as a non-mutating MTU backfill classification sprint. Classified the nine REF-CT1 Year-1 candidates against the current live registry: existing-unit mappings to `A45`, `A46`, `A47`, `A48`, `A49`, and `A51`; one merge/design candidate for normal/inferior goods around successor units `A17`/`D11`/`D33` with deprecated `D04` retained only as historical context; one deferred kink-in-collective-demand candidate; and one true missing simultaneous-shift reasoning operation for later governed review. Output: `references/data/sprints/CP.6c-mtu-backfill-classification.json` and `reports/reference-planning/CP.6c-mtu-backfill-classification.md`. No protected mutation, unit minting, target-exercise promotion, placeholder finalization, CP-6 closure, or Year-1 closure was authorized. CP.6d graph-heavy evidence upgrade and CP.6e focused `1.1.3` Part A re-review remain required. |
| CP.6b | Year-1 Target-Exercise Review | yes | Completed as a non-mutating target-exercise review/design sprint. Recorded 12 active-v5 Book 1 target-exercise records, 9 migrated records needing explicit v5 review artifacts, 3 placeholder records with draft gemengde-opgaven integration designs, 0 reviewed-final records, 0 target-exercise promotions, and 0 placeholder finalizations. Output: `references/data/sprints/CP.6b-target-exercise-review.json` and `reports/reference-planning/CP.6b-target-exercise-review.md`. CP-6 and Year 1 remain open; CP.6d graph-heavy evidence upgrade and CP.6e focused `1.1.3` Part A re-review remain required before any responsible closure proposal. |
...
- `CP.6d` active next: Book 1 graph-heavy evidence upgrade.
...
Next action: run `CP.6d Book 1 Graph-Heavy Evidence Upgrade`. CP.6a plus lesson-team L-CP6A cleared the `1.3.2`/`1.3.3` source/lesson mismatch with carried conditions, CP.6b recorded the non-final target-exercise review/design packet, and CP.6c classified the nine Year-1 backfill candidates against the current live MTU registry. CP.6c does not close CP-6 or Year 1: graph-heavy review evidence still needs upgrading, the focused `1.1.3` Part A re-review remains open, target exercises remain non-final, and the simultaneous-shift true missing operation needs later governed review before any mutation. The remaining CP-6 lanes are CP.6d graph-heavy evidence upgrade and CP.6e focused `1.1.3` Part A re-review. After CP.6e, the next inserted strategic lane is EX-0 through EX-2: design and pilot official exam-question ingestion with source-annex and answer-model traceability before Year-2 skeleton work depends on exam-operation claims. The remaining blocks on diagnostics, adaptive routing, mastery, sequencing, student-facing AI, summative use, PV projection, and PV machine promotion remain in force.
