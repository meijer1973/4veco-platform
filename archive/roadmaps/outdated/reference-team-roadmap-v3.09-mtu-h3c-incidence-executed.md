...
Roadmap version: `v3.09-mtu-h3c-incidence-executed`
...
|--------|------|-----------|---------------|
| MTU-H4 | Answer-Form MTUs And Question-Type Mapping | no | **ACTIVE OPERATIONAL NEXT ACTION.** Define a governed answer-form route for `bereken`, `leg uit`, `leg uit met voorbeeld`, `analyseer`, `arceer/grafisch`, `geef aan`, and related exam verbs, and map them to existing `question_type` values. Begin with a non-mutating planning/review packet that reads target exercises, exam-answer evidence, current `question_type` values, and existing MTUs before proposing any protected mutation. This row is contract/review work until a later CLI-governed mutation path is explicitly authorized. No protected reference mutation, target-exercise promotion, candidate writes, lesson output, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or student/product use is authorized. |
