...
Roadmap version: `v2.90-gate-mtu-h2b-cli-execution-review`
...
|--------|------|-----------|---------------|
| GATE-MTU-H2B | Solo q1-q3 CLI Execution Authorization Human Review | no | **ACTIVE OPERATIONAL NEXT ACTION.** Run the formal human review for the MTU-H2B execution-gate packet before any CLI execution. The gate should decide whether to authorize a later bounded CLI execution sprint for the unblocked lanes `F19`, `F20`, `A85-A93`, and the `A12` update, or require a `unit-add` dry-run wrapper first. Direct `A20` update/split execution is held because active target exercise `4.1.2` still uses `A20` in a given-MK context; `A20` must route to a later split/deprecate/replacement packet or an execution packet that also updates affected mappings and generator evidence. No hand edits to `references/machine/` or `references/external/`, CLI execution, unit minting, unit update execution, unit split execution, candidate writes, lesson-output mutation, CP-6/Year-1 closure, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or student-facing output are authorized before this gate closes. |
...
GATE-MTU-H2B Solo q1-q3 CLI execution authorization human review
...
Next action: run `GATE-MTU-H2B Solo q1-q3 CLI Execution Authorization Human Review` as the active operational stop point. `MTU-H2B` has completed a packet only and did not run CLI execution. The gate must decide whether the unblocked command set may move to a later bounded CLI execution sprint for `F19`, `F20`, `A85-A93`, and the `A12` update, or whether a `unit-add` dry-run wrapper is required first. Direct `A20` update/split execution remains blocked because target exercise `4.1.2` uses `A20` in a given-MK context; `A20` needs a later split/deprecate/replacement packet or an execution packet that also updates affected mappings and generator evidence. Until GATE-MTU-H2B closes, do not run CLI execution, hand-edit `references/machine/` or `references/external/`, mint units, execute unit updates or splits, create candidate storage, write candidates, mutate lesson output, close CP-6/Year-1, or authorize diagnostics, adaptive routing, mastery, sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or student/product use. GAME-UX-2 has already supplied the bounded checkpoint engine support; lesson-side broad companion scaling remains blocked by the lesson review/Scale Gate path, while the reference-side active lane is now GATE-MTU-H2B.
