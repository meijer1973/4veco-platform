...
Roadmap version: `v2.66-shared-skill-map-engine-tracking`
...
|--------|------|-----------|---------------|
| EX-3 | Exam Coverage Dashboard | no | **ACTIVE / REPORTING SPRINT NEXT.** GATE-EX2 closed as `pass_with_conditions` for classification and routing only. EX-3 must add JSON/Markdown coverage views for exam-ingestion state and record the reviewed EX-2 classifications: q3 `A61` support with `q3-calc-1` still `operation_registry_need`, q3/q15 `answer_skill_need`, q19 `A42` graph-shift candidate with `A45` weak support, and q19 source/graph gaps still blocking. No mutation, lesson output, CP-6/Year-1 closure, or student/product use. |
