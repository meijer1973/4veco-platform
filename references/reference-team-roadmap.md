...
| Sprint | Name | Completed | Current State |
|--------|------|-----------|---------------|
| BLUEPRINT-BOOK1-EDITION-BOUNDARY-1 | Book 1 Edition Boundary and Part A Compatibility Repair | no | Repair planning on PR #222 after merging current main. The owned preview/scaffolding clarification must explicitly inherit the Book 2+ Part A target-operation, seven-section, paper-route, and 55-minute contract; it must not manufacture `Covered` status, mastery, or an untargeted independent operation. Focused guardrails, current indexes, fresh teacher/lead review, exact-head CI, owner authorization, governed integration, and post-merge CI remain required. Book 1 and all lesson output stay unchanged; Issue #223 is planning-only until this prerequisite is integrated. |
