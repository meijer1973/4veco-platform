...
| GATE-TASK-FAMILY-1 | Structured Choice And Construction Task-Family Review | no | Human/lead review before the new structured choice or construction families are relied on by `REASON-STD-1`, `CHECK-SHORT-EXIT-2`, `SCALE-PROOF-3P`, or Scale Gate 1. Must inspect rendered output, feedback, keyboard/focus behavior, and target-proof boundaries. |
...
| GATE-TASK-FAMILY-1 | Structured Choice And Construction Task-Family Review | no | Review rendered output, schema/validation, feedback, keyboard/focus behavior, and target-proof boundaries before structured choice or construction families are relied on by reasoning migration, check implementation, or Scale Gate proof. |
...
Next action: proceed to `GATE-TASK-FAMILY-1` as the structured choice and
construction task-family review before relying on these families in product
routes, or to `GAME-ROUTE-AFFORDANCE-1` if route affordance is the priority and
no new task-family reliance is introduced. The closed `TASK-FAMILY-CLOZE-TILE-1`,
...
remains the next route-affordance implementation lane and may run alongside
task-family implementation if capacity allows, but `REASON-STD-1`,
`CHECK-SHORT-EXIT-2`,
`SCALE-PROOF-3P`, and Scale Gate 1 may not rely on the new task families
before `GATE-TASK-FAMILY-1` reviews rendered output, feedback, focus/keyboard
behavior, and target-proof boundaries.
...
`TASK-FAMILY-ORDER-1`, `TASK-FAMILY-SOURCE-1`, or `TASK-FAMILY-LABEL-1`.
