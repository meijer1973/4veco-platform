...
| Y1-GOLDEN-ROLLOUT-WAVE-1 | First-Three Golden Workflow Availability Wave | yes | Completed pending human review as a bounded rollout-control sprint for the first Year 1 Golden wave. Codifies that only the six current first-three split check surfaces are in scope, verifies existing rendered product-path and workflow evidence, records `1.1.4` as same-copy hygiene only, and names chapter `1.2` source authoring as a later sprint. Added CI-visible guard `check:y1-golden-rollout-wave-1`. No source-data edits, generated lesson output, engine/runtime behavior changes, protected reference mutation, target-exercise writes, route migration, product-route adoption, completion-language release, diagnostics, mastery/sequencing, PV, Scale Gate 1, broad product use, or student/product use. |
