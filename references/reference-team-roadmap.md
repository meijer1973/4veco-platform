...
|--------|------|-----------|---------------|
| CHECKSURFACE-113-EXEMPLAR-EXIT-1 | Excellent 1.1.3 Exit Ticket Implementation | no | Active bounded implementation/evidence sprint from `knowledge/113-excellent-exit-ticket-v3-package.zip`. Scope: replace the `1.1.3` exit-ticket source with the v3 independent-work design, extend shared task-shell support for interval-first graph reading, formula building, tolerant percent parsing, two-point table graph construction, and durable exemplar guidance, then regenerate Book 1 output through deploy only. Required end state is `hold_for_exemplar_review` with teacher-learning, student-experience, visual/interaction, testing/regression, and lead synthesis reviews still pending. No new completion language, product-route adoption, diagnostics, mastery/sequencing, PV, Scale Gate 1, broad product use, or student/product use is authorized. |
