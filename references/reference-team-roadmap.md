...
|--------|------|-----------|---------------|
| SINGLE-PR-DRY-RUN-REPAIR-1 | Canonical Single-PR Dry Run | yes | Completed in PR #220 at reviewed substantive head `870aa3f2...`: plain `--dry-run` is the canonical non-mutating pre-merge validation path, reports `validated_dry_run` without invented merge state, and proves behind/movement handling plus unchanged live behavior. Exact-head hosted CI, readiness and explicit owner authorization remain required; no workflow, bundle-runner, Lesson, product, Y1, protected-reference, authorization-model, authority, or merge change is included. |
