...
Roadmap version: `v2.61-ex0-exam-ingestion-contract`
...
|--------|------|-----------|---------------|
| EX-1 | Exam Source Annex And Answer Model Pilot | no | **ACTIVE / PILOT PREPARATION AFTER EX-0.** EX-0 defined the schema, review procedure, validator, and GATE-EX0 contract packet. Next step is to run the formal GATE-EX0 human review and then, only if authorized, ingest three VWO exam-question overlays as a bounded pilot: one calculation-heavy, one graph/source-heavy, and one reasoning/answer-model-heavy. Mark source-annex and answer-model gaps explicitly. No protected mutation, no external-source mutation, no unit minting, no CP-6 closure, no Year-1 closure, and no student/product authorization. |
...
- `EX-1` active next: three-question source-annex and answer-model extraction
  pilot after GATE-EX0 human review authorizes the contract.
...
EX-1 Exam source annex and answer model pilot active after GATE-EX0 review authorization
...
Next action: run `GATE-EX0 Exam Ingestion Contract` human review, then start `EX-1 Exam Source Annex And Answer Model Pilot` only if the gate authorizes the contract. CP.6a plus lesson-team L-CP6A cleared the `1.3.2`/`1.3.3` source/lesson mismatch with carried conditions, CP.6b recorded the non-final target-exercise review/design packet, CP.6c classified the nine Year-1 backfill candidates against the current live MTU registry, CP.6d upgraded the graph-heavy evidence ledger against the live post-L-CP6A lesson repo, CP.6e failed the first `1.1.3` Part A figure-order recheck, CP.6f cleared that focused figure-numbering blocker after lesson-team L-CP6E, and EX-0 defined the exam-ingestion contract without creating pilot data. CP-6 and Year 1 remain open: target exercises remain non-final, six graph-heavy quality refs still need later quality workflow before closure reliance, and the simultaneous-shift true missing operation needs later governed review before any mutation. EX-1 and EX-2 are the next bounded strategic lanes for exam-ingestion pilot/review design. The remaining blocks on diagnostics, adaptive routing, mastery, sequencing, student-facing AI, summative use, PV projection, and PV machine promotion remain in force.
