...
| Sprint | Name | Completed | Current State |
|--------|------|-----------|---------------|
| BLUEPRINT-BOOK1-EDITION-BOUNDARY-1 | Book 1 Edition Boundary and Part A Compatibility Repair | no | Substantive source is accepted at `bb21d53e...`; focused tests, the full suite, teacher review, and lead round 1 pass. The first round-2 recheck found evidence-ordering and stale-index-tail defects only, so result/command/index closure is in progress before a canonical round-2 verdict. Exact-head hosted CI, readiness, owner authorization, governed integration, and post-merge CI remain required. Book 1 and all lesson output stay unchanged; Issue #223 remains planning-only until this prerequisite is integrated. |
