...
| CHECK-SHORT-EXIT-2 | Implement Both Check Types For First Three Paragraphs | no | Direct human review returned `REVISE` with gate direction `hold_for_surface_repair` and additional direction `replan before the next human gate`; the gate is not closed and no product authority exists. The review accepted the administrative split-surface layer as useful but held the gate because the packet proved generated pages, screenshots, and authority flags rather than product end-state quality. `CHECKSURFACE-RESET-1` records comments, audit, findings JSON, and a checker. `GRAPH-CHECK-UX-1` is complete as the first surface repair: `1.1.3` advisory `Korte check` now uses graph/table task-shell interaction with source/table context blocks, `graph_construction_substitute`, `graph_reading`, and `table_value_selection` families, a graph workspace with visible grid and table-derived ticks, targeted wrong/retry feedback, route advice, generated Book 1 output, proof JSON, desktop/mobile/dark screenshots, and hard-fail checkers. `GRAPH-EXIT-UX-1` is complete as the second surface repair: `1.1.3` target-equivalent `Exit ticket` now uses split source/task graph workspace with an opt-in source/task layout, constrained scrollable source pane, sticky task question strip, graph workspace visible with source/table context in the desktop first viewport, source-scroll proof preserving task visibility, same-workspace graph line drawing, all three task checks matched, completion language held, generated Book 1 output, proof JSON, desktop/mobile/dark screenshots, and hard-fail checkers. `CHECK-ROUTE-COPY-1` is complete as the third surface repair: first-three paragraph landing pages now distinguish advisory `Korte check` route advice from `Exit ticket` end-check route with explicit badges, purpose copy, action text, data attributes, generated Book 1 output, proof JSON, desktop/mobile/dark screenshots, and a hard-fail checker. `VISUAL-QA-HARDEN-2` is complete as the fourth repair: it adds a consolidated product QA rubric, `visual-qa-harden2-proof.json`, product QA report, and hard-fail checker that guard `CSR1-F1` through `CSR1-F5` against graph/table, source/task, screenshot-only, label-only, and missing student-experience regressions. `CHECK-SURFACE-PREGATE-1` is complete as the fifth repair: it adds a five-minute product walkthrough, student-experience review, `check-surface-pregate1-proof.json`, readiness report, lead-review cycle, and checker proving the repaired surfaces are green for retry-packet preparation while not starting or closing the human gate. `GATE-CHECK-SHORT-EXIT-2-RETRY` packet preparation is complete: the retry direct-review packet, live-output evidence, review lab, bundle URLs, checker, lead-review cycle, and result are ready for direct human review comments; no retry comments, closure proposal, or gate closure exists yet. Required sequence before any retry gate closure: `CHECKSURFACE-RESET-1` -> `GRAPH-CHECK-UX-1` (complete) -> `GRAPH-EXIT-UX-1` (complete) -> `CHECK-ROUTE-COPY-1` (complete) -> `VISUAL-QA-HARDEN-2` (complete) -> `CHECK-SURFACE-PREGATE-1` (complete) -> `GATE-CHECK-SHORT-EXIT-2-RETRY` direct human review comments (next). Preserve reviewed `1.1.2` completion-language authority; keep new `1.1.1`/`1.1.3` completion language held. Do not proceed to `SCALE-PROOF-3P`, `GATE-PRODUCT-3P`, Scale Gate 1, product-route adoption, diagnostics, mastery/sequencing, PV, or student/product use before the retry gate authorizes it. |
...
Next action after `GATE-CHECK-SHORT-EXIT-2-RETRY` packet preparation: send the
retry packet for direct human review comments unless the roadmap owner
explicitly reorders. Do not start
...
`CHECK-SURFACE-PREGATE-1` are complete, and
`GATE-CHECK-SHORT-EXIT-2-RETRY` is next for direct human review comments.
`GATE-CHECK-SHORT-EXIT-2-RETRY` packet preparation is complete. Direct human
review comments are next.
`SCALE-PROOF-3P`,
