...
| Sprint | Name | Completed | Current State |
|--------|------|-----------|---------------|
| PART-A-TEXTBOOK-EXERCISE-STRUCTURE-1 | Book 2 Part A Exercise Authoring Contract | yes | Implementation complete with teacher-learning-quality `PASS` (14/14) and final lead-review `PASS`. The platform-only contract aligns the seven-section printed Part A sequence, backward target-operation design, optional faded support, the 2→4→5 route within a real 55-minute lesson, and the bonus/review distinction; a CI-wired mutation checker protects 10 active surfaces. Draft-PR publication, exact-head CI, and readiness routing remain before `READY_FOR_HUMAN_REVIEW`. No `4veco-lessen`, Book 1, Book 2 paragraph output, Part B redesign, protected reference mutation, schema-role migration, merge, or production authority. |
