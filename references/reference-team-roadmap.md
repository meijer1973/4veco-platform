...
Next action after `GRAPH-EXIT-UX-1`: proceed to `CHECK-ROUTE-COPY-1` unless the
...
`CHECKSURFACE-RESET-1`, `GRAPH-CHECK-UX-1`, and `GRAPH-EXIT-UX-1` are
complete, and `CHECK-ROUTE-COPY-1` is next. `SCALE-PROOF-3P`,
