...
Next action after `GRAPH-CHECK-UX-1`: proceed to `GRAPH-EXIT-UX-1` unless the
...
`CHECKSURFACE-RESET-1` and `GRAPH-CHECK-UX-1` are complete, and
`GRAPH-EXIT-UX-1` is next. `SCALE-PROOF-3P`, `GATE-PRODUCT-3P`, and Scale
Gate 1 still require their own rendered student-path proof and review.
