...
Next action after `VISUAL-QA-HARDEN-2`: proceed to `CHECK-SURFACE-PREGATE-1` unless the
...
`CHECK-ROUTE-COPY-1`, and `VISUAL-QA-HARDEN-2` are complete, and
`CHECK-SURFACE-PREGATE-1` is next.
`SCALE-PROOF-3P`,
