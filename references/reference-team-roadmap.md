...
Roadmap version: `v2.85-mtu-h1-operation-benchmark`
...
| MTU-H2 | Solo q1-q3 Canonical Micro-Case Review | no | **PLANNED AFTER MTU-H1.** Review the benchmarked Solo q1-q3 cases for possible CLI-governed MTU additions or refactors: verbal external-cost explanation, leg-uit-with-example answer form, pointwise TO, TVK from constant variable cost, unknown fixed costs from profit, scale-factor handling, granular MO routes, given-MK versus derived-MK, new monopoly price after Q*, and percentage price-change/pass-through distinction. MTU-H2 must prepare an explicit review/mutation plan before any `references/machine/` change; no direct mutation is authorized by MTU-H1. |
