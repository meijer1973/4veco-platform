...
Next action after `CHECK-ROUTE-COPY-1`: proceed to `VISUAL-QA-HARDEN-2` unless the
...
`CHECKSURFACE-RESET-1`, `GRAPH-CHECK-UX-1`, `GRAPH-EXIT-UX-1`, and
`CHECK-ROUTE-COPY-1` are complete, and `VISUAL-QA-HARDEN-2` is next.
`SCALE-PROOF-3P`,
