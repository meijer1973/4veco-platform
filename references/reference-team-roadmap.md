...
|--------|------|-----------|---------------|
| SINGLE-PR-DRY-RUN-REPAIR-1 | Canonical Single-PR Dry Run | no | Bounded governance repair in progress. Makes plain `--dry-run` the canonical non-mutating pre-merge validation path, reports `validated_dry_run` without invented merge state, and proves behind/movement handling plus unchanged live behavior. No workflow, bundle-runner, Lesson, product, Y1, protected-reference, authorization-model, authority, or merge change is included. |
