...
| CI-LF-HARDEN-1 | Line Ending And Generated Report Stability | yes | Closed 2026-06-06 as local PASS WITH FLAGS pending remote-publication proof. Added scoped LF policy for CI/build evidence surfaces, deterministic CRLF checker, Jest coverage, and `platform-ci` checker step. Rejected broad historical `reports/**/*.md/json` normalization after initial scan found 265 CRLF matches; carried flag: historical report-archive CRLF cleanup is separate follow-up only. Local checks pass, including `npm.cmd run check:platform`, scoped line-ending checker, scope language, report JSON, roadmap index, URL-index check, platform diff hygiene, and lessen diff hygiene. No generated lesson output, protected reference mutation, source-data mutation, target registries, candidate storage, PV outputs, product-route adoption, target-equivalent proof, diagnostics, mastery/sequencing, Scale Gate 1, or student/product use authorized. Next: `CI-EVIDENCE-1`; record remote `validate-platform` proof after branch push. |
| CI-EVIDENCE-1 | Cross-Repo Checkout Evidence Artifact | yes | Closed 2026-06-06 as local PASS pending remote artifact proof. Added `build-scripts/ci/platform-ci-evidence.js`, tests, workflow write/check step, and diagnostic artifact upload path `ci-artifacts/platform-ci-evidence.json`. Local evidence writes outside both checked-out repos and records platform SHA `ed12764ad39eb1290c899a9bee08edf6a77c126d` plus lessen SHA `15f823028522663ca62c8d6369af9c6ae0264efb` before final commit. No generated lesson output, protected reference mutation, source-data mutation, target registries, candidate storage, PV outputs, product-route adoption, target-equivalent proof, diagnostics, mastery/sequencing, PV, Scale Gate 1, or student/product use authorized. Next: `CI-GATE-PROOF-1`; inspect remote artifact after branch push. |
| CI-GATE-PROOF-1 | Human-Gate CI Proof Validator | yes | Closed 2026-06-06 as local PASS pending remote CI proof. Added `build-scripts/sprints/check-gate-ci-proof.js`, npm script `check:gate-ci-proof`, positive markdown/JSON fixtures, and negative fixtures for missing run ID, missing commit, non-success conclusion, vague waiver, run without reviewed commit, and local-only command-log proof. Optional `--remote` uses `gh api` without adding secrets. Historical gate packets were not rewritten. This hardens gate evidence only and authorizes no product readiness, lesson readiness, generated lesson output, protected reference mutation, source-data mutation, target-equivalent proof, diagnostics, mastery/sequencing, PV, Scale Gate 1, or student/product use. Next: `CI-GOVERNANCE-1` only after remote `validate-platform` proof is recorded. |
