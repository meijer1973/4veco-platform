...
Roadmap version: `v3.35-gate-l1-7b-q2-pass-with-flags`
...
|--------|------|-----------|---------------|
| L1.7B-Q2-COPY | Exact 1.1.2 Target-Equivalent Completion Copy Enablement | no | Newly authorized next as a later exact implementation packet after `GATE-L1.7B-Q2` closed PASS WITH FLAGS. Scope must be limited to reviewed `1.1.2 Percentages en indexcijfers` gate/copy flags or equivalent exact fields, the approved local non-summative copy `Je hebt laten zien dat je de eindopgave van deze paragraaf aankunt.`, regeneration only of affected `1.1.2` output through platform scripts, and evidence/checker updates proving all other paragraphs remain blocked. No broad exit-ticket scaling, `1.1.1` or `1.1.3` target-equivalent status, target-exercise registry promotion, CP-6/Year-1 reliance, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV, Scale Gate 1, or product use is authorized. |
...
Next action: prepare `L1.7B-Q2-COPY` as the exact reviewed `1.1.2` completion-copy enablement packet. Do not enable the approved copy until that packet or sprint is planned, validated, lead-reviewed, and authorized under its own scope. No diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV, Scale Gate 1, CP-6/Year-1 reliance, or product-wide use is authorized by `GATE-L1.7B-Q2`.
