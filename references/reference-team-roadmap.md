...
Roadmap version: `v2.84-game-ux2-exit-ticket-checkpoint`
...
|--------|------|-----------|---------------|
| LESSON-SCALE-1 | Lesson Companion Pre-Scale Gates Tracking | no | **TRACKING DEPENDENCY, NOT A REFERENCE EXECUTION SPRINT.** Lesson L1.7A closed PASS WITH FLAGS and rejected broad companion scaling. Lesson `L1.7B-C` is the closed contract-only exit-ticket companion contract: the old untracked draft was inspected as design evidence, the companion-completion contract and future implementation scope were recorded, and implementation remained paused because of source-control/product-boundary/shared-skill-map blockers. Lesson `L1.7B-R` produced its operational resume plan, platform support request, and stop decision; platform `GAME-UX-2` has now generated a bounded, source-controlled, non-summative checkpoint surface for `1.1.1` through platform scripts. Lesson L1.7B-R may resume student-experience and teacher-learning-quality review, then `GATE-L1.7B` must review product boundaries before Scale Gate 1. Lesson L1.7C-0 closed the shared skill-map engine contract, and platform `GAME-UX-1` implemented the shared skill-map runtime support. Lesson L1.7C closed PASS WITH FLAGS after focused recheck. Lesson L1.7D closed PASS WITH FLAGS after generator-owned landing IA cleanup. Lesson L2.0 closed PASS WITH FLAGS after human review: it defined the house-style baseline, PASS WITH FLAGS meaning, quality-ref status standard, exit-ticket readiness checklist, screenshot/QA expectations, and game-row flag dispositions; platform support routes `1.1.4 Gemengde opgaven` to `Oefen gemengd` rather than a Verdiep-only landing. EX-7 is resolved as a validator/dry-run CLI implementation only; reference/product planning must not treat the lesson companion pipeline as scale-approved until L1.7B-R review, GATE-L1.7B, minimal curriculum-versioning readiness, and Scale Gate 1 resolve while preserving blocks on diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, and PV machine promotion. |
...
| Product Gate | Re-Evaluate R10-R13 Product Surfaces | no | Blocked. Diagnostics, adaptive routing, student-facing AI, teacher cockpit deployment, automatic sequencing, mastery, and summative uses remain out of scope until data-quality gates justify them. Lesson L1.7A explicitly rejected broad companion scaling; L1.7B is split into closed contract-only `L1.7B-C` and implementation-resume `L1.7B-R`; L1.7C-0 closed the shared skill-map contract; platform `GAME-UX-1` implemented shared route/view-model support without authorizing product use; platform `GAME-UX-2` generated one bounded checkpoint surface for `1.1.1` without broad scaling or product-use authorization; lesson L1.7C closed PASS WITH FLAGS after consuming that shared route in the game row; lesson L1.7D closed PASS WITH FLAGS after generator-owned landing IA cleanup; lesson L2.0 closed PASS WITH FLAGS after defining house-style and fixing consolidation landing semantics; GATE-EX5 closed PASS WITH CONDITIONS as design-contract review only; EX-6 completed validator/CLI planning without candidate writes; GATE-EX6 closed PASS WITH CONDITIONS for validator/dry-run CLI implementation only; and EX-7 implemented validators and dry-run-only CLIs without candidate storage or writes. Broad lesson-companion scaling remains blocked until L1.7B-R reviews the generated checkpoint surface, GATE-L1.7B accepts its product boundaries, and Scale Gate 1 establishes remaining curriculum-versioning and game-row scaling quality. |
