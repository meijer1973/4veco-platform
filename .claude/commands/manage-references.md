...
| File | Purpose |
|---|---|
| `economie-terminologie.md` | **Canonical Dutch economic terms.** All student-facing content must use terms from this file. |
...
9. **`terms` must resolve to `economie-terminologie.md` entries.** Validator rejects unknown terms. Canonical set is extracted from headings, bolded leads, and pipe-table rows (split on `/` or `,`); em-dash placeholder accepted in the number column; gloss-stripped forms added alongside full forms.
