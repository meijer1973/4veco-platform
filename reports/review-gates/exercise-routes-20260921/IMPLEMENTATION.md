...
The three independent probe/export scripts are retained unchanged as `.py.txt` / `.cjs.txt` evidence attachments. Their original execution directory was the task's external `review/` folder beside the paired worktrees; they are not executable entrypoints in this report directory.
