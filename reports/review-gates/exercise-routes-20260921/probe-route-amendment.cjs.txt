// Independent review probe. All mutations occur in this review fixture, not the repositories.
'use strict';
const fs = require('fs'), path = require('path'), assert = require('assert'), crypto = require('crypto');
const platform = path.resolve(__dirname, '../4veco-platform');
...
const fixture = path.join(__dirname, 'amendment-probe-fixture');
...
const cases=[];
function test(name,fn) {reset();fn();cases.push({name,result:'passed'});}
...
reset();
const hashes=Object.fromEntries(['build-scripts/references/books34-route-amendment.js','build-scripts/references/books34-selected-structure.js',...files].map(f=>[f,crypto.createHash('sha256').update(fs.readFileSync(path.join(platform,f))).digest('hex')]));
fs.writeFileSync(path.join(__dirname,'amendment-probe-results.json'),JSON.stringify({scope:'read-only source review; fixture mutations only',cases,hashes},null,2)+'\n');
