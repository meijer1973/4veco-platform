'use strict';
// Read-only transport adapted from the fully read, published 224-QC-CURRENT runner.
...
for(const[k,root]of [['PLATFORM',P],['LESSEN',L]]){A.equal(path.resolve(process.env['FOURVECO_'+k+'_ROOT']),root);A.match(process.env['FOURVECO_'+k+'_SOURCE_REF'],/^[a-f0-9]{40}$/);A.equal(process.env['FOURVECO_'+k+'_SOURCE_BRANCH'],'agent/book2-22-consistency-review-20260906');}
