...
const relocated = path.join(__dirname, prefix + 'evidence/specialist-r5');
const resolveMedia = p => path.join(relocated, p.replaceAll('\\','/').split('/213-specialist-r5/')[1]);
...
for (const doc of media.documents) {
  ...
  assert.equal(digest(path.join(relocated, doc.kind + '.pdf')), doc.pdf_sha256);
  ...
  for (const [page, sha] of Object.entries(doc.pages)) {
    assert.equal(digest(resolveMedia(page)), sha);
...
for (const [file, sha] of Object.entries(media.grayscale)) assert.equal(digest(resolveMedia(file)), sha);
