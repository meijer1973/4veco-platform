...
for(const [k,root]of [['PLATFORM',P],['LESSEN',L]]){A.equal(path.resolve(process.env['FOURVECO_'+k+'_ROOT']),root);A.match(process.env['FOURVECO_'+k+'_SOURCE_REF'],/^[a-f0-9]{40}$/);A.equal(process.env['FOURVECO_'+k+'_SOURCE_BRANCH'],'agent/book2-234-plan-review-20260906');}
