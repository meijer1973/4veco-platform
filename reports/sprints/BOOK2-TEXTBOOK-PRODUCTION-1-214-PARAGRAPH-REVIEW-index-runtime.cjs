'use strict';
// Owned transport for exact read-only inventories; shared index policy remains byte-exact.
...
for(const [k,root]of [['PLATFORM',P],['LESSEN',L]]){A.equal(path.resolve(process.env['FOURVECO_'+k+'_ROOT']),root);A.match(process.env['FOURVECO_'+k+'_SOURCE_REF'],/^[a-f0-9]{40}$/);A.equal(process.env['FOURVECO_'+k+'_SOURCE_BRANCH'],'agent/book2-214-paragraph-review-20260906');}
if(require.main!==module){
 ...
 const rows=[];for(const [label,root,key]of [['platform',P,'PLATFORM'],['lessen',L,'LESSEN']]){
  ...
  // NUL prevents core.quotepath representation from being mistaken for real filenames.
