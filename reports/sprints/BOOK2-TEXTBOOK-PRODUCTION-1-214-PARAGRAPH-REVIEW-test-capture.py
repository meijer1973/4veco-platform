...
env=dict(os.environ);env.update(PYTHONIOENCODING='utf-8',PYTHONDONTWRITEBYTECODE='1',FOURVECO_PLATFORM_ROOT=str(P),FOURVECO_PLATFORM_SOURCE_REF=git(P,'rev-parse','HEAD').decode().strip(),FOURVECO_PLATFORM_SOURCE_BRANCH='agent/book2-214-paragraph-review-20260906',FOURVECO_LESSEN_ROOT=str(L),FOURVECO_LESSEN_SOURCE_REF=git(L,'rev-parse','HEAD').decode().strip(),FOURVECO_LESSEN_SOURCE_BRANCH='agent/book2-214-paragraph-review-20260906')
...
result={'argv':argv,'cwd':str(P),'source_commit':'a62bc517c5c13b4f93bf836ad5bd37b9843e7c55','source_sha256':sha(source.read_bytes()),'review_controller_commit':env['FOURVECO_PLATFORM_SOURCE_REF'],'paired':{k:v for k,v in env.items() if k.startswith('FOURVECO_')},'PATH_unchanged':True,'PATH_sha256':sha(env['PATH'].encode()),'start':start,'finish':datetime.datetime.now(datetime.timezone.utc).isoformat(),'exit_code':exit_code,'streams':records,'capture_errors':errors,'tests':13 if not errors and re.search(r'Ran 13 tests in [0-9.]+s',records['stderr']['tail_utf8']) else None,'native_rebuilds_in_this_capture':0,'temporary_fixtures':'Exact reviewed test TemporaryDirectory cleanup, not foreign existing paths','independent_review':'Source tests supporting separate personal review; not verdict'}
...
print(json.dumps({'status':'PASS','tests':13,'exit_code':exit_code,'complete_lossless_streams':{k:{'raw_bytes':v['raw_bytes'],'gzip_bytes':v['gzip_bytes'],'raw_sha256':v['raw_sha256']} for k,v in records.items()},'native_rebuilds_in_this_capture':0,'independent_review':'Source tests supporting separate personal review; not verdict'}))
