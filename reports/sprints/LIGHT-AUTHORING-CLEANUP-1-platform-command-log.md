# LIGHT-AUTHORING-CLEANUP-1 local platform command log

Command: `npm.cmd run check:platform`. Exit code: 0. The emitted missing-file
and FAIL lines below are intentional negative fixtures; the Jest summary is
passing. Source repair06c7925b occurred while this local run was in progress,
so final exact-head CI is tracked separately.
...
Tests:       8 skipped, 1889 passed, 1897 total
Snapshots:   0 total
Time:        628.467 s
Ran all test suites.
```
