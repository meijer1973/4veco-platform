# Lead Review Assignment: TASK-FAMILY-SENTENCE-1
...
Review the `sentence_builder` shared task-shell implementation before sprint
closure. This is a runtime-only implementation sprint with report-fixture proof
and no generated lesson output.
...
| Engine contract review | lead reviewer | `sentence_builder` validates token banks, accepted sequences, distractors, no-reuse default, and exact response shape. | required |
| UI/UX review | lead reviewer | Rendered markup exposes fragment bank, ordered sequence zone, add/remove/reorder controls, and one feedback region. | required |
...
| Test review | lead reviewer/tool | Focused Jest and custom checker pass and include strict-shape, wrong-order, missing/extra token, duplicate-use, and wrapper checks. | required |
