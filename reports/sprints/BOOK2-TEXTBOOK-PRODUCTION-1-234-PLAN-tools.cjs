...
function run(label,cwd,command,args,allowed=[0],record=true){
 ...
 if(record)save(label+'-process',result);console.log(label+': '+r.status);assert(allowed.includes(r.status),label+' failed; original record retained');return result;
}
...
if(mode==='baseline'){
 ...
 claims(false,false);for(const [root,allowed]of [[P,own],[L,f=>f===REL]]){const pending=[...new Set([...list(root,'diff','--name-only','-z'),...list(root,'diff','--cached','--name-only','-z'),...list(root,'ls-files','--others','--exclude-standard','-z')])];assert(pending.every(allowed));for(let i=0;i<pending.length;i+=30)git(root,'add','--',...pending.slice(i,i+30));run((root===P?'platform':'lessons')+'-whitespace',root,'git',['diff','--cached','--check'],[0],false);}console.log(JSON.stringify(custody()));
}else if(mode==='indexes'){run('paired-indexes',P,'node',['build-scripts/reports/github-agent-index.js'],[0],false);run('url-index',P,'node',['build-scripts/sprints/emit-url-index.js'],[0],false);}
else if(mode==='final'){
 run('index-freshness',P,'node',['build-scripts/reports/check-agent-index-freshness.js'],[0],false);run('url-freshness',P,'node',['build-scripts/sprints/emit-url-index.js','--check'],[0],false);claims(true,false);
