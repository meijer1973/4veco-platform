# Sprint TASK-FAMILY-CLOZE-1: Baseline
...
`TASK-FAMILY-CHOICE-1` has accepted the `cloze_text` contract but did not
authorize implementation. The shared task shell currently supports adjacent
families, including `cloze_tile_select`, `sentence_builder`, and
`formula_builder`, plus established deterministic families such as `choice`,
`table_value_selection`, and structured response families.

`cloze_text` is not yet declared as a shared task-shell family, and typed inline
blank responses are not yet validated, rendered, collected by wrappers, or
...
Proceed to planning review for a runtime-only `cloze_text` implementation if
