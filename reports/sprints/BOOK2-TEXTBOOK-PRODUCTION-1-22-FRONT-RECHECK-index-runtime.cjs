'use strict';
// Read-only transport adapted from the fully read, published C22 consistency-review runner.
// No shared source edit; actual NUL filenames are the independent comparator.
...
for(const[k,root]of [['PLATFORM',P],['LESSEN',L]]){A.equal(path.resolve(process.env['FOURVECO_'+k+'_ROOT']),root);A.match(process.env['FOURVECO_'+k+'_SOURCE_REF'],/^[a-f0-9]{40}$/);A.equal(process.env['FOURVECO_'+k+'_SOURCE_BRANCH'],'agent/book2-22-front-recheck-20260906');}
if(require.main!==module){
 ...
 const rows=[];for(const[label,root,key]of [['platform',P,'PLATFORM'],['lessen',L,'LESSEN']]){
  ...
  A.equal(v.source_commit,process.env['FOURVECO_'+key+'_SOURCE_REF']);A.equal(v.source_branch,process.env['FOURVECO_'+key+'_SOURCE_BRANCH']);
  const raw=original('git',['ls-tree','-r','--name-only','-z',v.source_commit,'--'],{cwd:root,maxBuffer:128*1024*1024}),skip=new Set(v.skipped_directories),expected=raw.toString('utf8').split('\0').filter(Boolean).filter(f=>!f.split('/').some(x=>skip.has(x))).sort();
