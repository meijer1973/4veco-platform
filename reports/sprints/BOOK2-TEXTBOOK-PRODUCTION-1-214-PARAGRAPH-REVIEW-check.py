...
def env():
    e=dict(os.environ)
    e.update(PYTHONDONTWRITEBYTECODE='1',PYTHONIOENCODING='utf-8',NODE_PATH='C:/wt/book2-part-a-production-20260905/4veco-platform/node_modules')
...
def scan(rev,route):
    ...
    save('scan-next-'+route+'-'+rev[1:],{'status':'PASS','namespace':n,'actor':'paragraph_214_builder','task':'BOOK2-TEXTBOOK-PRODUCTION-1-214-PARAGRAPH-REVIEW','branch':BR})
def native(rev,route):
    n=js(P/(PRE+'scan-next-'+route+'-'+rev[1:]+'.json'))['namespace'];dest=Path(n['proof_root'])
    ...
    packages=[Path(e['NODE_PATH'])/name/'package.json' for name in ['js-yaml','sharp']]
    invocation={'actor':'paragraph_214_builder','task':'BOOK2-TEXTBOOK-PRODUCTION-1-214-PARAGRAPH-REVIEW','controller_commit':anchor,'argv':argv,'cwd':str(P),'PATH':e['PATH'],'PATH_inherited_unchanged':True,'NODE_PATH_child_only':e['NODE_PATH'],'existing_packages':[{'path':str(f),'sha256':sha(raw(f)),'version':js(f)['version']} for f in packages],'paired':{k:v for k,v in e.items() if k.startswith('FOURVECO_')},'start':datetime.datetime.now(datetime.timezone.utc).isoformat()}
    save('invocation-before-'+route+'-'+rev[1:],invocation)
