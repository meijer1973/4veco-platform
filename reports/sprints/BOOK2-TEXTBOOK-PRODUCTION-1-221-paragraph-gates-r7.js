'use strict';
// Reviewer-owned wrapper: preserve Windows path arguments, use actual git scope.
const fs = require('fs');
...
for (const [name, cwd, base, lane] of [
  ...
  const ownBase = name==='platform' ? 'f48cda9341213d12e4a92b9839d3fb18f07ce004' : 'a1996c0faef2eff4605617cab6d38887858949b8';
  ...
  for (const p of ownPaths) {
    const allowed = name==='platform'
      ? p.startsWith('reports/sprints/BOOK2-TEXTBOOK-PRODUCTION-1-221-') || ['reports/url-index.md','AGENT_GITHUB_ENTRY.md','RESEARCH_AGENT_MAP.md','RESEARCH_AGENT_MAP_REFERENCES.md','reports/github-agent-index-lessen.json','reports/github-agent-index-lessen.md','reports/github-agent-index-platform.json','reports/github-agent-index-platform.md'].includes(p)
      : p===path.relative(lessons,path.join(folder,'2.2.1-review.md')).replace(/\\/g,'/');
...
fs.writeFileSync(path.join(__dirname,'BOOK2-TEXTBOOK-PRODUCTION-1-221-paragraph-scope-r7.json'),
