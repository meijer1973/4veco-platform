...
const P=path.resolve(__dirname,'../..'),L=path.resolve(P,'../4veco-lessen'),prefix='reports/sprints/BOOK2-TEXTBOOK-PRODUCTION-1-214-232-INPUT-REVIEW';
const BP='eafd9785801e8ec0b689d05dc0f92835f7f06ed0',BL='1cf1c1f972f196791fb37f6bbee523b7a2e3b676',branch='agent/book2-214-232-input-review-20260906',task='BOOK2-TEXTBOOK-PRODUCTION-1-214-232-INPUT-REVIEW',actor='paragraph_224_builder';
...
function run(label,cwd,command,args,allowed=[0],record=true){
 const childEnv={...env(),NODE_PATH:'C:/wt/book2-part-a-production-20260905/4veco-platform/node_modules'},started_at=new Date().toISOString();
...
if(mode==='baseline'){
 ...
 const history=list(P,'ls-tree','-r','--name-only','-z',BP).filter(f=>f.startsWith('reports/sprints/BOOK2-TEXTBOOK-PRODUCTION-1-214-232-PRODUCTION-RELEASE-')&&!f.startsWith(prefix+'-')).map(f=>({path:f,raw_sha256:sha(read(P,f)),bytes:read(P,f).length}));
 const author=JSON.parse(fs.readFileSync('C:/wt/book2-234-plan-review-20260906/4veco-platform/reports/sprints/BOOK2-TEXTBOOK-PRODUCTION-1-234-PLAN-REVIEW-baseline.json'));
 const instructions=author.instructions.map(v=>{const root=v.repository==='4veco-lessen'?L:P;assert.equal(sha(read(root,v.path)),v.raw_sha256,v.path);return {...v,reading_attribution:'This same actor previously personally read these complete unchanged instructions; current paired AGENTS/review/didactic/testing/schema/teacher/student/lane rereads supplement those retained personal reads.'};});
 for(const f of ['build-scripts/templates/template-textbook-paragraph-plan.md'])instructions.push({repository:'4veco-platform',path:f,raw_sha256:sha(read(P,f)),reading_attribution:'Previously personally read complete and exact current bytes rebound; no other actor reading substituted.'});
 ...
 assert.deepEqual(list(P,'diff','--name-only','-z','HEAD^..HEAD').sort(),[...indexes].sort());console.log(JSON.stringify({pair,strict:strict(),custody:custody(),native_scopes:scopes(false),terminal_four_index_only:true,verdict:'INPUT_PASS',production_release:false,rendered_review:false},null,2));
