...
describe('TaskShellEngine', () => {
    ...
    test('validates first-class context blocks and task references', () => {
        const taskSet = {
            schema_version: 1,
            title: 'Context-first source task',
            contextBlocks: [
                {
                    id: 'bron-schaarste',
                    type: 'markdown',
                    label: 'Bron 1',
                    title: 'Keuze in droge zomer',
                    sourceRef: 'Reconstructed local review source, 2026',
                    body: 'Een gemeente moet kiezen tussen sproeien van sportvelden en extra drinkwaterbuffers.'
                },
                {
                    id: 'tabel-water',
                    type: 'table',
                    ...
                    sourceRef: 'Reconstructed local review source, 2026',
                    columns: ['Activiteit', 'Gebruik per dag'],
                    rows: [
                        ['Sportvelden sproeien', '120.000 liter'],
                        ['Drinkwaterbuffer aanvullen', '180.000 liter']
                    ],
                    caption: 'De tabel geeft de beschikbare alternatieven weer.'
                },
                {
                    id: 'figuur-water',
                    type: 'graph',
                    label: 'Figuur 1',
                    title: 'Vraag naar water',
                    sourceRef: 'Reconstructed local review source, 2026',
                    altText: 'Dalende lijn in een prijs-hoeveelheidgrafiek voor water.',
                    svg: '<svg viewBox="0 0 120 80" role="img"><path d="M10 65 L105 15" /></svg>',
                    caption: 'Reconstructed graph, not a copied bitmap.'
                }
            ],
            tasks: [
                {
                    ...baseTask({
                        ...
                        family: 'source_value_selection',
                        skillLabel: 'Bronwaarden kiezen',
                        prompt: 'Kies de twee alternatieven uit de bron.',
                        contextRefs: ['bron-schaarste', 'tabel-water'],
                        interaction: {
                            valueBankLabel: 'Bronwaarden',
                            roleLabel: 'Rol',
                            values: [
                                { id: 'sportvelden', label: 'Sportvelden sproeien', kind: 'answer', sourceLabel: 'activiteit' },
                                { id: 'drinkwater', label: 'Drinkwaterbuffer aanvullen', kind: 'answer', sourceLabel: 'activiteit' },
                                { id: 'regen', label: 'Regenverwachting', kind: 'distractor', distractorFor: 'sportvelden' }
                            ],
                            roles: [
                                { id: 'option-a', label: 'alternatief A' },
                                { id: 'option-b', label: 'alternatief B' }
                            ]
                        },
                        expected: {
                            kind: 'source_value_selection',
                            selections: [
                                { valueId: 'sportvelden', role: 'option-a' },
                                { valueId: 'drinkwater', role: 'option-b' }
                            ],
                            partialFeedback: 'practice_only'
                        }
                    })
                ...
                {
                    ...baseTask({
                        id: 'context-graph-label',
                        family: 'label_placement',
                        skillLabel: 'Grafieklabels plaatsen',
                        prompt: 'Plaats de aslabels bij de gereconstrueerde grafiek.',
                        contextRefs: ['figuur-water'],
                        interaction: {
                            labelBankLabel: 'Labelbank',
                            targetRegionLabel: 'Grafiekvlak',
                            placementLabel: 'Geplaatste labels',
                            visual: {
                                kind: 'coordinate_plane',
                                title: 'Vraaggrafiek',
                                description: 'Een assenstelsel voor prijs en hoeveelheid.'
                            },
                            labels: [
                            ...
                            targets: [
                                { id: 'y-as', label: 'Verticale as', description: 'Plaats hier het prijslabel.', kind: 'answer', targetRole: 'axis', x: 12, y: 28 },
                                { id: 'x-as', label: 'Horizontale as', description: 'Plaats hier het hoeveelheidlabel.', kind: 'answer', targetRole: 'axis', x: 70, y: 84 },
                                { id: 'caption', label: 'Bijschrift', description: 'Dit is niet het assenlabel.', kind: 'distractor', targetRole: 'structure_part', distractorFor: 'x-as', x: 76, y: 16 }
                            ]
                        },
                        expected: {
                            kind: 'label_placement',
        ...
        expect(TaskShellEngine.validateTaskSet(taskSet)).toBe(true);
    });

    test('rejects broken context contracts before rendering', () => {
        const taskSet = {
            schema_version: 1,
        ...
        expect(() => TaskShellEngine.validateTaskSet({
            ...taskSet,
            tasks: [baseTask({ id: 'context-task' })]
        })).toThrow(/contextRefs/);

        expect(() => TaskShellEngine.validateTaskSet({
            ...taskSet,
            contextBlocks: [
                ...taskSet.contextBlocks,
                {
        ...
        })).toThrow(/not referenced by any task/);
        ...
        expect(() => TaskShellEngine.validateTaskSet({
            ...
            contextBlocks: [{
                ...
                type: 'graph',
                label: 'Figuur 1',
                title: 'Grafiek zonder alt',
                sourceRef: 'Reconstructed local review source, 2026',
                svg: '<svg viewBox="0 0 10 10"></svg>'
            }],
            tasks: [baseTask({ id: 'context-task', contextRefs: ['figuur-1'] })]
        })).toThrow(/altText/);
        ...
        expect(() => TaskShellEngine.validateTaskSet({
            ...
            contextBlocks: [{
                ...
                label: 'Figuur 1',
                title: 'Bitmap in SVG',
                altText: 'Een gekopieerde afbeelding.',
                sourceRef: 'Reconstructed local review source, 2026',
                svg: '<svg viewBox="0 0 10 10"><image href="copy.png" /></svg>'
            }],
            tasks: [baseTask({ id: 'context-task', contextRefs: ['figuur-1'] })]
        })).toThrow(/copied bitmap/);
        ...
        expect(() => TaskShellEngine.validateTaskSet({
            ...
            contextBlocks: [{
                ...
                label: 'Bron 1',
                sourceRef: 'Reconstructed local review source, 2026',
                body: 'Een bron met onstabiel id.'
            }]
        })).toThrow(/kebab-case/);
    });
