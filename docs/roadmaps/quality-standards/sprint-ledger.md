...
| `INSPECT-2A` | closed / pass | Corrections-only profile adjustment before schema design. Adds evidence-finality, target-equivalent proof, subject-material OP0, title/source reconciliation, diagnostic-report status, and per-category product/school boundary language. | `archive/sprints/INSPECT-2A/INSPECT-2A-sprint-plan.md`; `archive/sprints/INSPECT-2A/INSPECT-2A-correction-packet.md`; `archive/sprints/INSPECT-2A/INSPECT-2A-validation-log.md`; `archive/sprints/INSPECT-2A/INSPECT-2A-lead-review-round2.md`; `archive/sprints/INSPECT-2A/INSPECT-2A-closure-log.md` | this commit | Dutch profile remains `status: draft`; `review_status` moved cautiously to `draft_adjusted_for_schema_design`. No schema, validator, evidence pack, lesson-output change, quality-ref integration, dashboard gate, Scale Gate work, overlay, or compliance claim. |
...
| --- | --- | --- |
| `INSPECT-3` | Report-only evidence schema design for inspection/quality evidence records, only after human review accepts INSPECT-2A. | not authorised |
