...
Roadmap version: `v2.87-gate-mtu-h2-pass-with-conditions`
...
|--------|------|-----------|---------------|
| MTU-H2A | Solo q1-q3 CLI-Mutation Planning | no | **ACTIVE OPERATIONAL NEXT ACTION.** Prepare a bounded planning packet for the GATE-MTU-H2-approved Solo q1-q3 lanes only. The plan must name exact live unit IDs or proposed new unit IDs, schema-valid CLI specs, proposed `unit-add` / `unit-update` / `unit-add-dep` / split-or-supplement command sequences, rollback and audit-log evidence, validation commands, and proof required before any execution gate. H2A is planning-only unless a later gate explicitly authorizes exact CLI execution; it must not hand-edit `references/machine/` or `references/external/`, mint units, write candidates, mutate lesson output, close CP-6/Year-1, authorize diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or authorize student-facing output. |
