...
Roadmap version: `v3.50-step-ordering-runtime`
...
| TASK-FAMILY-SOURCE-1 | Source Value And Chain Builder Implementation | no | Add `source_value_selection` and `source_chain_builder` so students select multiple source/table/graph values and label them as old/new, x/y, price/quantity, cause/effect, or source -> value -> operation -> answer chains. |
| TASK-FAMILY-LABEL-1 | Label-Placement Task-Family Implementation | no | Add `label_placement` for graph, table, formula, and representation tasks such as axes, lines, intersections, units, index labels, and curve-shift components. Requires visual and keyboard proof. |
...
| TASK-FAMILY-SOURCE-1 | Source Value And Chain Builder Implementation | no | Implement multi-value source selection plus source -> value -> operation -> answer chain construction for graph/table/source tasks, including old/new, x/y, price/quantity, cause/effect, and unit/label roles. |
| TASK-FAMILY-LABEL-1 | Label-Placement Task-Family Implementation | no | Implement label-placement interactions for graphs, formulas, source structures, axes, lines, intersections, and representation components. Requires visual, keyboard/focus, mobile, and dark-mode proof. |
...
Next action: proceed to `TASK-FAMILY-SOURCE-1` as the next construction
...
`TASK-FAMILY-MULTI-1`, and `TASK-FAMILY-ORDER-1` runtimes may not be adopted in generated lesson
routes before a later adoption sprint supplies product-route rendered proof.
