...
Roadmap version: `v3.45-cloze-tile-runtime`
...
| TASK-FAMILY-SENTENCE-1 | Sentence Builder Task-Family Implementation | no | Add `sentence_builder` so students build economic reasoning sentences or causal chains from fragments. Use for `leg uit`, index-point explanations, curve-shift reasoning, surplus/welfare reasoning, and exam-answer phrasing where fragments force the required structure. |
...
| TASK-FAMILY-SENTENCE-1 | Sentence Builder Task-Family Implementation | no | Implement sentence/chain building from word or fragment banks for economic reasoning. It must force the causal or answer-form structure rather than merely assembling plausible words. |
...
Next action: proceed to `TASK-FAMILY-SENTENCE-1` or
`TASK-FAMILY-FORMULA-1` as the next construction implementation sprint, or to
`GAME-ROUTE-AFFORDANCE-1` if route affordance is the priority. The closed
`TASK-FAMILY-CLOZE-TILE-1` runtime may not be adopted in generated lesson
