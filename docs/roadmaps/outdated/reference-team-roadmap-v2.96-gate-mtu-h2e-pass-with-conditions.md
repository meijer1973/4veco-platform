...
Roadmap version: `v2.96-gate-mtu-h2e-pass-with-conditions`
...
|--------|------|-----------|---------------|
| MTU-H2F | Conditional Solo q1-q3 CLI Execution | no | **ACTIVE OPERATIONAL NEXT ACTION.** Execute only the GATE-MTU-H2E reviewed command set for `A12`, `A88`, `A89`, `A90`, `A92`, and `A93`, with `A20` held. Use the reference CLI only, print and compare each extracted JSON spec before execution, run `A12 --dry-run` first and prove `A2.11` remains, fresh-check `A88`/`A89`/`A90`/`A92`/`A93` are absent and `A12`/`A20` are present, confirm no `A20` command exists, refresh generator-readiness records, and prove missing `GEN_A88`, `GEN_A89`, `GEN_A90`, `GEN_A92`, and `GEN_A93` are generator-blocked/not-yet-interactive or otherwise not exposed to student-facing skill-tree/PV routes. No hand edits to `references/machine/` or `references/external/`, candidate writes, lesson-output mutation, target-exercise promotion, CP-6/Year-1 closure, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or student/product use are authorized. |
