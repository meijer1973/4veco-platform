...
Roadmap version: `v2.63-ex1-exam-ingestion-pilot`
...
|--------|------|-----------|---------------|
| EX-2 | Exam-to-MTU Mapping Review Gate | no | **ACTIVE / HUMAN REVIEW GATE NEXT.** EX-1 created three bounded non-mutating pilot exam-ingestion overlays and a pilot validator. EX-2 must review the pilot requirements and classify them as existing MTU, weak MTU, missing MTU, merge/split candidate, operation-registry need, PV/graph need, answer-skill need, source-annex gap, answer-model extraction gap, or defer. No mutation before human review; no CP-6/Year-1 closure or student/product use. |
...
- `EX-1` completed: three-question source-annex and answer-model extraction
  ...
  blocking.
- `EX-2` active next: human-reviewed exam-to-MTU mapping gate before any
  mutation.
...
EX-2 Exam-to-MTU mapping review gate active next
...
Next action: start `EX-2 Exam-to-MTU Mapping Review Gate`. CP.6a plus lesson-team L-CP6A cleared the `1.3.2`/`1.3.3` source/lesson mismatch with carried conditions, CP.6b recorded the non-final target-exercise review/design packet, CP.6c classified the nine Year-1 backfill candidates against the current live MTU registry, CP.6d upgraded the graph-heavy evidence ledger against the live post-L-CP6A lesson repo, CP.6e failed the first `1.1.3` Part A figure-order recheck, CP.6f cleared that focused figure-numbering blocker after lesson-team L-CP6E, EX-0 defined the exam-ingestion contract, GATE-EX0 closed as `pass_with_conditions`, and EX-1 created the three bounded pilot exam-ingestion overlays with q19 graph/source gaps carried as blocking. CP-6 and Year 1 remain open: target exercises remain non-final, six graph-heavy quality refs still need later quality workflow before closure reliance, and the simultaneous-shift true missing operation needs later governed review before any mutation. EX-2 is the next bounded strategic lane for human-reviewed exam-to-MTU mapping. The remaining blocks on diagnostics, adaptive routing, mastery, sequencing, student-facing AI, summative use, PV projection, and PV machine promotion remain in force.
