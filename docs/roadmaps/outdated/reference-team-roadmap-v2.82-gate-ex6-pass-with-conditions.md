...
Last strategic update: 2026-05-26
Roadmap ID: `references-team-roadmap`
Roadmap version: `v2.82-gate-ex6-pass-with-conditions`
...
|--------|------|-----------|---------------|
| EX-7 | Validator And Dry-Run CLI Implementation | no | **ACTIVE IMPLEMENTATION SPRINT AFTER GATE-EX6; VALIDATORS AND DRY-RUN CLIS ONLY.** GATE-EX6 closed as `pass_with_conditions` and authorizes this bounded implementation lane only. EX-7 may implement validators for operation candidates, answer-skill candidates, and source-annex extraction overlays; implement dry-run-only CLIs; create temporary non-persistent test-only fixtures; and add rejection tests for GATE-EX6 conditions. EX-7 must not create `references/data/exam-ingestion/operation-candidates.json`, `references/data/exam-ingestion/answer-skill-candidates.json`, or `references/data/exam-ingestion/source-annex-extraction-overlays.json`; write candidate records; execute q19 source-annex or graph-object extraction; mutate protected references, external sources, machine references, units, operation registries, answer-skill registries, PV/graph records, target exercises, or lesson output; close CP-6/Year 1; or authorize diagnostics, adaptive routing, mastery, sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or student/product use. Conditions: distinguish weak vs rejected unit evidence; reject empty/vague q19 reconstructability fields; hard-fail any write mode without a later gate; keep `A15` rejected for q3 and `A45` weak-only for q19; keep q3/q15 answer-skill needs visible. |
| GAME-UX-2 | Exit Ticket Checkpoint Engine MVP | no | **LESSON SUPPORT REQUEST RECEIVED; STILL NOT ACTIVE WHILE EX-7 IS THE ACTIVE REFERENCE LANE.** Lesson L1.7B-R created `archive/sprints/L1.7B-R/L1.7B-R-platform-support-request.md` and stopped before implementation. GAME-UX-2 remains the required platform support lane for a source-controlled, non-summative exit-ticket checkpoint engine or wrapper, but it needs an explicit later start decision or waiver. When active, it must consume the shared skill-map compact checkpoint mode from `GAME-UX-1`, support paragraph target-skill selection, neutral feedback, retry/self-check affordances, and generator-owned landing-page `Check` slot integration. It must not import the untracked prototype directly, use mastery/pass/score/evidence/adaptive-focus language, read adaptive payloads for routing, expose internal MTU codes to students, or authorize diagnostics, adaptive routing, mastery, sequencing, AI, summative use, PV projection, PV machine promotion, lesson-output mutation, or student/product use. |
| LESSON-SCALE-1 | Lesson Companion Pre-Scale Gates Tracking | no | **TRACKING DEPENDENCY, NOT A REFERENCE EXECUTION SPRINT.** Lesson L1.7A closed PASS WITH FLAGS and rejected broad companion scaling. Lesson `L1.7B-C` is the closed contract-only exit-ticket companion contract: the untracked prototype was inspected as design evidence, the companion-completion contract and future MVP scope were recorded, and implementation remained paused because of source-control/product-boundary/shared-skill-map blockers. Lesson `L1.7B-R` has now produced its operational resume plan, platform support request, and stop decision; the MVP is not built and GATE-L1.7B remains blocked until GAME-UX-2 or an approved equivalent produces a generated, reviewable checkpoint surface. Lesson L1.7C-0 closed the shared skill-map engine contract, and platform `GAME-UX-1` implemented the shared skill-map runtime support. Lesson L1.7C closed PASS WITH FLAGS after focused recheck. Lesson L1.7D closed PASS WITH FLAGS after generator-owned landing IA cleanup. Lesson L2.0 closed PASS WITH FLAGS after human review: it defined the house-style baseline, PASS WITH FLAGS meaning, quality-ref status standard, exit-ticket readiness checklist, screenshot/QA expectations, and game-row flag dispositions; platform support routes `1.1.4 Gemengde opgaven` to `Oefen gemengd` rather than a Verdiep-only landing. GATE-EX6 is resolved as `pass_with_conditions` for validator/dry-run CLI implementation only; reference/product planning must not treat the lesson companion pipeline as scale-approved until L1.7B-R MVP output, GATE-L1.7B, minimal curriculum-versioning readiness, and EX-7 or an explicit waiver are resolved while preserving blocks on diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, and PV machine promotion. |
...
| Product Gate | Re-Evaluate R10-R13 Product Surfaces | no | Blocked. Diagnostics, adaptive routing, student-facing AI, teacher cockpit deployment, automatic sequencing, mastery, and summative uses remain out of scope until data-quality gates justify them. Lesson L1.7A explicitly rejected broad companion scaling; L1.7B is split into closed contract-only `L1.7B-C` and active implementation-resume `L1.7B-R`; L1.7C-0 closed the shared skill-map contract; platform `GAME-UX-1` implemented shared route/view-model support without authorizing product use; lesson L1.7C closed PASS WITH FLAGS after consuming that shared route in the game row; lesson L1.7D closed PASS WITH FLAGS after generator-owned landing IA cleanup with `Check` hidden until reviewed exit-ticket output exists; lesson L2.0 closed PASS WITH FLAGS after defining house-style and fixing consolidation landing semantics; GATE-EX5 closed PASS WITH CONDITIONS as design-contract review only; EX-6 completed validator/CLI planning without candidate writes; and GATE-EX6 closed PASS WITH CONDITIONS for validator/dry-run CLI implementation only. Broad lesson-companion scaling remains blocked until L1.7B-R produces a boundary-safe checkpoint MVP, GATE-L1.7B accepts its product boundaries, and Scale Gate 1 establishes remaining curriculum-versioning, EX-7 status or waiver, and game-row scaling quality. |
...
for `GAME-UX-2` is now present, but GAME-UX-2 remains inactive while GATE-EX6
is the active reference gate unless a human waiver explicitly permits the
platform support lane. GATE-L1.7B and Scale Gate 1 remain blocked until a
...
- `EX-7` active next: implement validators and dry-run-only CLIs with no
...
- `LESSON-SCALE-1` tracks the lesson-side pre-scale dependency: L1.7A rejected
  ...
  GATE-L1.7B, minimal curriculum-versioning readiness, EX-7 or an explicit
  waiver, and Scale Gate 1.
...
SYNC-2 exit-ticket and scale-gate roadmap precision completed: L1.7B split into L1.7B-C contract-only and L1.7B-R future MVP resume; GATE-L1.7B and GAME-UX-2 inserted; Scale Gate 1 prerequisites clarified
...
EX-7 Validator and dry-run CLI implementation active next, with no candidate storage, candidate writes, or q19 extraction execution
LESSON-SCALE-1 lesson companion pre-scale gates tracked: L1.7A closed with broad scaling rejected; L1.7B-C recorded the exit-ticket companion contract and paused MVP implementation; L1.7B-R is now the active lesson boundary-safe implementation resume; GATE-L1.7B remains required before scale; L1.7C-0 closed the shared skill-map contract; GAME-UX-1 implemented platform shared skill-map runtime support; L1.7C closed PASS WITH FLAGS after focused recheck; L1.7D closed PASS WITH FLAGS; L2.0 closed PASS WITH FLAGS with game-row flag disposition, exit-ticket readiness, house-style policy, and consolidation landing proof recorded; EX-7 and Scale Gate 1 remain before broad companion/product scaling
GAME-UX-1 shared skill-map engine architecture completed for the closed lesson L1.7C-0 contract
GAME-UX-2 exit-ticket checkpoint engine MVP future platform support for L1.7B-R
...
Next action: start `EX-7 Validator And Dry-Run CLI Implementation` as the active reference lane after `GATE-EX6` closed as `pass_with_conditions` for validator/dry-run CLI implementation only. CP.6a plus lesson-team L-CP6A cleared the `1.3.2`/`1.3.3` source/lesson mismatch with carried conditions, CP.6b recorded the non-final target-exercise review/design packet, CP.6c classified the nine Year-1 backfill candidates against the current live MTU registry, CP.6d upgraded the graph-heavy evidence ledger against the live post-L-CP6A lesson repo, CP.6e failed the first `1.1.3` Part A figure-order recheck, CP.6f cleared that focused figure-numbering blocker after lesson-team L-CP6E, EX-0 defined the exam-ingestion contract, GATE-EX0 closed as `pass_with_conditions`, EX-1 created the three bounded pilot exam-ingestion overlays with q19 graph/source gaps carried as blocking, GATE-EX2 closed as `pass_with_conditions` for classification and routing only, EX-3 reported the reviewed classifications, EX-4 prepared mutation-planning artifacts, GATE-EX4 closed as `pass_with_conditions` for routing/design only, EX-5 prepared the operation/answer-skill/q19 extraction contract, GATE-EX5 accepted that contract only for later bounded validator/CLI planning, EX-6 added future-storage schemas, a validator/CLI implementation plan, a read-only checker, and the GATE-EX6 packet, and GATE-EX6 authorized EX-7 only for validators, dry-run-only CLIs, temporary non-persistent test fixtures, and rejection tests. Lesson L1.7A rejected broad companion scaling after green current validators, L1.7B has been split into closed contract-only `L1.7B-C` and active safe implementation resume `L1.7B-R`, L1.7C-0 closed the shared skill-map contract, GAME-UX-1 implemented the platform shared skill-map runtime support without lesson-output or product-use authorization, lesson L1.7C closed PASS WITH FLAGS after consuming the shared route in the game row, lesson L1.7D closed PASS WITH FLAGS after generator-owned landing IA cleanup and human review, SYNC-2 clarified the exit-ticket/Scale Gate prerequisites, and lesson L2.0 closed PASS WITH FLAGS with house-style policy, game-row flag disposition, exit-ticket readiness, and consolidation landing proof recorded. EX-7 must not create candidate storage, write candidate records, execute q19 source-annex or graph-object extraction, mutate protected references, mutate external sources, mutate machine references, mint units, mutate operation-registry or answer-skill storage, mutate PV/graph records, mutate lesson output, or authorize student/product use. CP-6 and Year 1 remain open: target exercises remain non-final, six graph-heavy quality refs still need later quality workflow before closure reliance, and the simultaneous-shift true missing operation needs later governed review before any mutation. Lesson-side broad companion scaling remains blocked until L1.7B-R, GATE-L1.7B, EX-7 or an explicit waiver, and Scale Gate 1 establish checkpoint review evidence and remaining scaling quality. The remaining blocks on candidate-storage writes, protected mutation, external-source mutation, machine-reference mutation, unit minting, operation-registry mutation, answer-skill mutation, q19 extraction execution, PV/graph mutation, lesson-output mutation, diagnostics, adaptive routing, mastery, sequencing, student-facing AI, summative use, PV projection, and PV machine promotion remain in force.
