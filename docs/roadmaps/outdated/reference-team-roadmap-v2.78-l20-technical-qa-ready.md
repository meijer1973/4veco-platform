...
Roadmap version: `v2.78-l20-technical-qa-ready`
...
|--------|------|-----------|---------------|
| GATE-EX5 | Operation And Answer-Skill Contract Human Review | no | **ACTIVE HUMAN REVIEW GATE.** EX-5 completed the design-only operation/answer-skill/q19 extraction contract and prepared `reports/review-gates/GATE-EX5-operation-answer-skill-contract/review-packet.md`. The gate must decide whether the proposed `references/data/exam-ingestion/` overlay storage paths, operation-candidate fields, answer-skill fields, q19 source-annex/graph-object extraction fields, validator/CLI preconditions, rollback, audit evidence, and routing facts are adequate for a later bounded tooling or extraction-planning sprint. GATE-EX5 must not authorize protected mutation, external-source mutation, machine-reference mutation, unit minting, operation-registry mutation, answer-skill mutation, q19 extraction execution, PV/graph mutation, target-exercise promotion, lesson-output mutation, CP-6 closure, Year-1 closure, diagnostics, adaptive routing, mastery, sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or student-facing output. |
| GAME-UX-2 | Exit Ticket Checkpoint Engine MVP | no | **PLATFORM SUPPORT FOR LESSON L1.7B-R; FUTURE, NOT ACTIVE BEFORE GATE-EX5 AND L2.0 HUMAN-REVIEW ROUTING.** Implement or adapt a source-controlled, non-summative exit-ticket checkpoint engine or checkpoint wrapper only after the lesson-side L1.7B-R resume path is authorized. It must consume the shared skill-map compact checkpoint mode from `GAME-UX-1`, support paragraph target-skill selection, neutral feedback, retry/self-check affordances, and generator-owned landing-page `Check` slot integration. It must not import the untracked prototype directly, use mastery/pass/score/evidence/adaptive-focus language, read adaptive payloads for routing, expose internal MTU codes to students, or authorize diagnostics, adaptive routing, mastery, sequencing, AI, summative use, PV projection, PV machine promotion, lesson-output mutation, or student/product use. |
| LESSON-SCALE-1 | Lesson Companion Pre-Scale Gates Tracking | no | **TRACKING DEPENDENCY, NOT A REFERENCE EXECUTION SPRINT.** Lesson L1.7A closed PASS WITH FLAGS and rejected broad companion scaling. Lesson `L1.7B-C` is now the closed contract-only exit-ticket companion contract: the untracked prototype was inspected as design evidence, the companion-completion contract and future MVP scope were recorded, and implementation remained paused because of source-control/product-boundary/shared-skill-map blockers. Lesson `L1.7B-R` remains required for a boundary-safe source-controlled MVP before Scale Gate 1, followed by lesson gate `GATE-L1.7B` for product-boundary review. Lesson L1.7C-0 closed the shared skill-map engine contract, and platform `GAME-UX-1` implemented the shared skill-map runtime support. Lesson L1.7C closed PASS WITH FLAGS after focused recheck. Lesson L1.7D closed PASS WITH FLAGS after generator-owned landing IA cleanup. Lesson L2.0 is now technical-QA green / ready for human review: it defines the house-style baseline, PASS WITH FLAGS meaning, quality-ref status standard, exit-ticket readiness checklist, screenshot/QA expectations, and game-row flag dispositions; platform support routes `1.1.4 Gemengde opgaven` to `Oefen gemengd` rather than a Verdiep-only landing. Reference/product planning must not treat the lesson companion pipeline as scale-approved until L2.0 closes, L1.7B-R, GATE-L1.7B, minimal curriculum-versioning readiness, and GATE-EX5 status are explicitly resolved or waived by human decision while preserving blocks on diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, and PV machine promotion. |
...
| Product Gate | Re-Evaluate R10-R13 Product Surfaces | no | Blocked. Diagnostics, adaptive routing, student-facing AI, teacher cockpit deployment, automatic sequencing, mastery, and summative uses remain out of scope until data-quality gates justify them. Lesson L1.7A explicitly rejected broad companion scaling; L1.7B is split into closed contract-only `L1.7B-C` and future implementation `L1.7B-R`; L1.7C-0 closed the shared skill-map contract; platform `GAME-UX-1` implemented shared route/view-model support without authorizing product use; lesson L1.7C closed PASS WITH FLAGS after consuming that shared route in the game row; lesson L1.7D closed PASS WITH FLAGS after generator-owned landing IA cleanup with `Check` hidden until reviewed exit-ticket output exists; and lesson L2.0 is now technical-QA green / ready for human review after defining house-style and fixing consolidation landing semantics. Broad lesson-companion scaling remains blocked until L2.0 closes, L1.7B-R produces a boundary-safe checkpoint MVP, GATE-L1.7B accepts its product boundaries, and Scale Gate 1 establishes remaining curriculum-versioning, GATE-EX5, and game-row scaling quality. |
...
- `EX-3` active next: coverage dashboards and reports first, CLI-backed
  mutation only after later explicit review authority.
- `EX-4` planned after EX-3 evidence: governed mutation planning only after
  explicit human authorization.
- `LESSON-SCALE-1` tracks the lesson-side pre-scale dependency: L1.7A has
  rejected broad scaling; `L1.7B-C` closed the exit-ticket contract-only lane;
  `L1.7B-R` remains the future boundary-safe MVP resume; `GATE-L1.7B` remains
  the required exit-ticket product-boundary review; L1.7C-0 closed the shared
  skill-map contract; platform `GAME-UX-1` implemented the shared skill-map
  runtime support; and lesson L1.7C/L1.7D closed PASS WITH FLAGS. Broad
  companion/product scaling must still wait for L2.0, L1.7B-R,
  GATE-L1.7B, game-row flag disposition, minimal curriculum-versioning
  readiness, GATE-EX5 status, and Scale Gate 1.
...
GATE-EX5 Operation and answer-skill contract human review active next
LESSON-SCALE-1 lesson companion pre-scale gates tracked: L1.7A closed with broad scaling rejected; L1.7B-C recorded the exit-ticket companion contract and paused MVP implementation; L1.7B-R remains future boundary-safe implementation; GATE-L1.7B remains required before scale; L1.7C-0 closed the shared skill-map contract; GAME-UX-1 implemented platform shared skill-map runtime support; L1.7C closed PASS WITH FLAGS after focused recheck; L1.7D closed PASS WITH FLAGS; L2.0 is technical-QA green / ready for human review with game-row flag disposition and consolidation landing proof recorded; GATE-EX5 status and Scale Gate 1 remain before broad companion/product scaling
