...
Last strategic update: 2026-05-25
Roadmap ID: `references-team-roadmap`
Roadmap version: `v2.80-gate-ex5-pass-with-conditions`
...
| Product Gate | Re-Evaluate R10-R13 Product Surfaces | no | Blocked. Diagnostics, adaptive routing, student-facing AI, teacher cockpit deployment, automatic sequencing, mastery, and summative uses remain out of scope until data-quality gates justify them. Lesson L1.7A explicitly rejected broad companion scaling; L1.7B is split into closed contract-only `L1.7B-C` and active implementation-resume `L1.7B-R`; L1.7C-0 closed the shared skill-map contract; platform `GAME-UX-1` implemented shared route/view-model support without authorizing product use; lesson L1.7C closed PASS WITH FLAGS after consuming that shared route in the game row; lesson L1.7D closed PASS WITH FLAGS after generator-owned landing IA cleanup with `Check` hidden until reviewed exit-ticket output exists; lesson L2.0 closed PASS WITH FLAGS after defining house-style and fixing consolidation landing semantics; and GATE-EX5 closed PASS WITH CONDITIONS as design-contract review only. Broad lesson-companion scaling remains blocked until L1.7B-R produces a boundary-safe checkpoint MVP, GATE-L1.7B accepts its product boundaries, and Scale Gate 1 establishes remaining curriculum-versioning, EX-6 operation/answer-skill planning, and game-row scaling quality. |
...
for `GAME-UX-2` is now present, but GAME-UX-2 remains inactive while EX-6 is
the allowed next reference lane unless a human waiver explicitly permits the
...
- `EX-6` active next: validator/CLI implementation planning with no candidate
  writes, q19 extraction execution, protected mutation, lesson-output mutation,
...
- `LESSON-SCALE-1` tracks the lesson-side pre-scale dependency: L1.7A rejected
  ...
  GATE-L1.7B, minimal curriculum-versioning readiness, EX-6 planning or an
