...
Last strategic update: 2026-05-27
Roadmap ID: `references-team-roadmap`
Roadmap version: `v2.88-mtu-h2a-cli-mutation-planning`
...
|--------|------|-----------|---------------|
| GATE-MTU-H2A | Solo q1-q3 CLI-Mutation Plan Human Review | no | **ACTIVE OPERATIONAL NEXT ACTION.** Run the formal human review for the MTU-H2A exact ID and CLI-spec planning packet before any CLI execution or registry mutation. Evidence: `reports/mtu-hardening/solo-q1-q3-cli-mutation-plan.*` and `reports/review-gates/GATE-MTU-H2A-cli-mutation-plan/review-packet.*`. The gate must decide whether the proposed `F19/F20`, `A85-A93`, and `A12`/`A20` update plan is adequate, whether the current `unit-add` no-dry-run limitation is acceptable or needs a wrapper, and what later bounded execution packet may be prepared. The gate does not itself authorize hand edits to `references/machine/` or `references/external/`, unit minting, unit update execution, candidate writes, lesson-output mutation, CP-6/Year-1 closure, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or student-facing output. |
