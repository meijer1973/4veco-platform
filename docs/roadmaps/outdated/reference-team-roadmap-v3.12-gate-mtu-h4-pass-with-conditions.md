...
Roadmap version: `v3.12-gate-mtu-h4-pass-with-conditions`
...
- `LESSON-SCALE-1` tracks the lesson-side pre-scale dependency: L1.7A rejected
  ...
  companion/product scaling must still wait for MTU-H4A, GAME-UX-3A,
...
Next action: start `MTU-H4A Answer-Form CLI-Mutation Planning Packet` after
GATE-MTU-H4 closed PASS WITH CONDITIONS at reviewed remote commit
`38a032346db9a6a1dcb247d52afce745c48863f7`. MTU-H4A may prepare exact
answer-form unit IDs/specs, held-lane treatment, source-boundary language,
rollback expectations, and validation proof only. It must carry the held Type
4 / motiveer / classificatie lane, treat `bron` as a source-use modifier plus
underlying answer form, keep graph/draw/shade planning-only until stronger
mapping evidence, keep analysis/evaluation held, and keep q3/q15 EX overlays
visible. No hand edits to `references/machine/` or `references/external`, unit
minting, candidate storage creation, candidate writes, target-exercise
mutation, generated projection refresh, lesson-output mutation, diagnostics,
adaptive routing, mastery/sequencing, student-facing AI, summative use, PV
projection, PV machine promotion, Scale Gate 1, or student/product use are
authorized. GAME-UX-3A remains the shared task-type UX foundation dependency,
but must not claim live answer-form MTUs or target-exercise readiness until
