...
Roadmap version: `v3.17-gate-mtu-h4b-pass-with-conditions`
...
|--------|------|-----------|---------------|
| MTU-H4C | Answer-Form Bounded CLI Execution | no | **ACTIVE NEXT SPRINT AFTER GATE-MTU-H4B.** Execute only the reviewed H4B `unit-add` commands for `A96` bereken, `A97` uitleg_dat, `A98` uitleg_of, `A99` leg uit met voorbeeld, `A80` noem/geef-aan with split-if-needed condition, and `A81` bron gebruiken only as source-use modifier plus underlying answer form. Quality floor: write an operational sprint plan before execution; run final preflight proving `A80`, `A81`, and `A96`-`A99` absent, `A71` held, `A100` invalid, candidate storage absent, and no target-exercise `question_type`/`answer_form` fields; print and log every exact spec and command hash; explicitly accept the `unit-add` no-dry-run limitation using simulated catalog validation and exact command review; execute the six commands; rebuild/check generator readiness; mark `GEN_A80`, `GEN_A81`, and `GEN_A96`-`GEN_A99` implemented or generator-blocked/not-yet-interactive; prove no student-facing skill-tree, PV, lesson, diagnostic, adaptive, mastery, sequencing, AI, summative, or product route exposes generator-blocked answer-form units; run the full validator stack; and record rollback evidence. This sprint may mutate only the protected MTU catalog through `unit-add.js` for the accepted commands. It must not create candidate storage, write candidate records, write target-exercise fields, mint graph/Type 4/analysis lanes, consume `A71`, treat `A100` as valid, refresh generated projections as a source mutation side effect, produce lesson output, authorize diagnostics/adaptive/mastery/sequencing/student-facing AI/summative/PV/product use, or advance Scale Gate 1. |
| LESSON-SCALE-1 | Lesson Companion Pre-Scale Gates Tracking | no | **TRACKING DEPENDENCY, NOT A REFERENCE EXECUTION SPRINT.** Lesson L1.7A closed PASS WITH FLAGS and rejected broad companion scaling. Closed lesson prerequisites now include `L1.7B-C`, `L1.7B-R`, `GATE-L1.7B`, `L1.7C-0`, `L1.7C`, `L1.7D`, `L2.0`, `L1.7C-MATH`, `L1.7B-MAP`, `L1.7B-P23`, and `SPEC-ET-1`. Platform `GAME-UX-1` supplied shared skill-map runtime support; platform `GAME-UX-2` supplied source-controlled `1.1.1` checkpoint support. `L1.7C-MATH` restored scoped skill-tree math as primary `Rekenen`; `L1.7B-MAP` fixed checkpoint metadata and kept `targetReadinessEvidence: false`; `L1.7B-P23` accepted the stop/handoff because choice-only checks cannot honestly cover `1.1.2` calculation/index or `1.1.3` graph/table operation chains; `SPEC-ET-1` corrected the stable specs so exit tickets are target-equivalent proof tasks, not readiness-to-try checks. SYNC-4 makes the shared task-type UI part of the product end-state and adds the operational engine proof track. GATE-MTU-H4A closed PASS WITH CONDITIONS for later bounded execution-packet preparation only; MTU-H4B prepared the non-mutating execution packet; GATE-MTU-H4B closed PASS WITH CONDITIONS authorizing only `MTU-H4C` bounded execution. Reference/product planning must not treat the lesson companion pipeline as scale-approved until `MTU-H4C`, `EX-LESSON-1`, `GAME-UX-3A`, `ENGINE-OP-1`, `SKILLMAP-OP-1`, `GRAPH-UX-2`, `MATH-UX-2`, `REASON-UX-2`, `GAME-ARCH-1`, `GATE-ENGINE-1`, lesson `L1.7B-Q2`, `GATE-L1.7B-Q2`, `REV-STD-1`, minimal curriculum-versioning readiness, and Scale Gate 1 resolve or are explicitly waived while preserving blocks on diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, and PV machine promotion. |
| EX-LESSON-1 | Exam-Ingestion End-State Integration | no | **LESSON PRODUCT INTEGRATION DEPENDENCY BEFORE EXAM-TARGET SCALE RELIANCE.** Update lesson-roadmap gates, paragraph-plan requirements, and later lesson-build handoffs so official CvTE and CvTE-derived target exercises trace prompt, source annexes, graphs/tables/figures, correction model, point allocation, answer-construction requirements, concepts, calculations, graph/table/source operations, reasoning operations, and answer-writing requirements into the explanation, skill-map route, shared task shell, target-equivalent exit ticket, and answer model. This row authorizes no protected reference mutation, generated lesson output, target-exercise mutation, diagnostics, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, Scale Gate 1, or student/product use. |
| GAME-UX-3A | Shared Task-Type UX Foundation | no | **PLANNED DEPENDENCY AFTER GATE-MTU-H4A CLOSURE OR WITH H4A CONDITIONS AND SPEC-ET-1 TARGET-EQUIVALENT REQUIREMENTS VISIBLE.** Expand the checkpoint/task engine into a reusable task shell for target-equivalent exit tickets, checkpoint-only local checks, graph/table practice, math/calculation practice, and exam-style answer-form requirements. Required task families: numeric input, calculation/work capture, final-answer entry, unit/notation field, short constructed response, table-value selection, graph reading, point placement, and graph-construction substitute. The shell must support neutral feedback, retry/self-check, screenshot QA, keyboard/focus QA, and no mastery/diagnostic/sequencing/summative/product-use claims. GAME-UX-3A may use the H4/H4A routing decisions as design input, but it must not claim live answer-form MTUs or target-equivalent exit-ticket proof until later governed source mutations, implementation evidence, and GATE-L1.7B-Q2 close. This row authorizes no source-data mutation, generated lesson output, `Check` activation, target-exercise promotion, protected reference mutation, candidate writes, diagnostics, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, Scale Gate 1, or broad companion scaling. |
...
- `LESSON-SCALE-1` tracks the lesson-side pre-scale dependency: L1.7A rejected
  ...
  L1.7C-MATH, L1.7B-MAP, and L1.7B-P23 have closed with carried flags. Broad
  companion/product scaling must still wait for MTU-H4C, GAME-UX-3A,
  ENGINE-OP-1, SKILLMAP-OP-1, GRAPH-UX-2, MATH-UX-2, REASON-UX-2,
...
MTU-H4C answer-form bounded CLI execution
GAME-UX-3A shared task-type UX foundation
...
Next action: start `MTU-H4C Answer-Form Bounded CLI Execution` under the GATE-MTU-H4B closure conditions. Before execution, write the MTU-H4C sprint plan and baseline; run final preflight proving `A80`, `A81`, and `A96`-`A99` absent, `A71` held, `A100` invalid, candidate storage absent, and no target-exercise `question_type`/`answer_form` fields; print and log each reviewed spec and command hash; explicitly accept the `unit-add` no-dry-run limitation using the simulated validation control; then execute only the six reviewed commands. After execution, rebuild/check generator readiness, mark `GEN_A80`, `GEN_A81`, and `GEN_A96`-`GEN_A99` implemented or generator-blocked/not-yet-interactive, prove no student-facing route exposes them, and run the full validator stack. MTU-H4C must not create candidate storage, write candidate records, write target-exercise fields, mint graph/Type 4/analysis lanes, consume `A71`, treat `A100` as valid, refresh generated projections as a source mutation side effect, produce lesson output, authorize diagnostics/adaptive/mastery/sequencing/student-facing AI/summative/PV/product use, or advance Scale Gate 1. GAME-UX-3A remains the shared task-type UX foundation dependency, but must not claim live answer-form MTUs or target-equivalent exit-ticket proof until later governed source mutations and evidence close.
