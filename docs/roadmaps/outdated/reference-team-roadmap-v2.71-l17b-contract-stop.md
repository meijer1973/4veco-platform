...
Roadmap version: `v2.71-l17b-contract-stop`
...
| LESSON-SCALE-1 | Lesson Companion Pre-Scale Gates Tracking | no | **TRACKING DEPENDENCY, NOT A REFERENCE EXECUTION SPRINT.** Lesson L1.7A closed PASS WITH FLAGS and rejected broad companion scaling. Lesson L1.7B then recorded the exit-ticket companion-completion contract, inspected the untracked `knowledge/exit-ticket-game-1.1.1.zip` prototype as design evidence only, and paused MVP implementation because the prototype is not source-controlled and uses mastery/score/pass/evidence/adaptive-focus semantics. Lesson roadmap now makes L1.7C-0 the active dependency before any exit-ticket MVP implementation, followed by L1.7C, L1.7D, L2.0, and Scale Gate 1. Reference/product planning must not treat the lesson companion pipeline as scale-approved until those lesson-side gates exist and preserve blocks on diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, and PV machine promotion. |
| GAME-UX-1 | Shared Skill-Map Engine Architecture | no | **PLATFORM SUPPORT FOR LESSON L1.7C-0.** L1.7B confirmed that exit-ticket MVP implementation depends on this architecture. Build or refactor one shared skill-map engine consumed by the reasoning, calculation, and graphical practice engines, and define compact checkpoint mode for the later exit-ticket surface. It must expose aspect-filtered skill subsets, recommended-next-skill logic, locked/open/completed state, prerequisite path rendering, compact/route/full display modes, and progress/stars display. It must avoid unfiltered all-skill views by default and must not authorize adaptive diagnostics, mastery, automatic sequencing, student-facing AI, summative use, PV projection, or PV machine promotion. |
...
| Product Gate | Re-Evaluate R10-R13 Product Surfaces | no | Blocked. Diagnostics, adaptive routing, student-facing AI, teacher cockpit deployment, automatic sequencing, mastery, and summative uses remain out of scope until data-quality gates justify them. Lesson L1.7A explicitly rejected broad companion scaling; L1.7B recorded the companion-set contract but paused exit-ticket MVP implementation. Broad lesson-companion scaling remains blocked until L1.7C-0, L1.7C, L1.7D, L2.0, the resumed boundary-safe exit-ticket MVP, and Scale Gate 1 establish the shared skill-map architecture, game-row quality rubric, landing-page information architecture, and checkpoint review evidence. |
...
- `LESSON-SCALE-1` and `GAME-UX-1` track the lesson-side pre-scale dependency:
  L1.7A has rejected broad scaling, and broad companion/product scaling must
  still wait for the resumed boundary-safe exit-ticket MVP, lesson L1.7C-0,
  L1.7C, L1.7D, L2.0, and a shared skill-map architecture decision. L1.7B
  recorded the companion contract but paused implementation.
...
LESSON-SCALE-1 lesson companion pre-scale gates tracked: L1.7A closed with broad scaling rejected; L1.7B recorded the exit-ticket companion contract and paused MVP implementation; L1.7C-0, L1.7C, L1.7D, L2.0, and a resumed boundary-safe exit-ticket MVP remain before broad companion/product scaling
GAME-UX-1 shared skill-map engine architecture support for lesson L1.7C-0
