...
Roadmap version: `v3.14-exit-ticket-target-equivalent-spec`
...
|--------|------|-----------|---------------|
| GATE-MTU-H4A | Answer-Form CLI-Mutation Plan Human Review | no | **ACTIVE HUMAN REVIEW GATE AFTER MTU-H4A.** Review the non-mutating H4A planning packet before any answer-form execution packet is prepared. The gate must decide whether the exact proposed IDs `A80`, `A81`, and `A96`-`A99` are acceptable despite A-domain ID pressure, confirm `A100` is invalid and `A71` remains unused, approve or revise `A96` bereken, `A97`/`A98`/`A99` explanation lanes, `A80` noem/geef-aan, and `A81` bron-as-modifier, keep graph/draw/shade, Type 4 motiveer/classificatie, and analysis/evaluation held unless stronger evidence is named, keep q3/q15 EX answer-skill overlays visible, and decide whether only a later bounded execution packet may be prepared. Remote-before-review publication is required. No protected reference mutation, unit minting, candidate storage creation, candidate writes, target-exercise mutation, generated projection refresh, lesson output, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, Scale Gate 1, or student/product use is authorized. |
| LESSON-SCALE-1 | Lesson Companion Pre-Scale Gates Tracking | no | **TRACKING DEPENDENCY, NOT A REFERENCE EXECUTION SPRINT.** Lesson L1.7A closed PASS WITH FLAGS and rejected broad companion scaling. Closed lesson prerequisites now include `L1.7B-C`, `L1.7B-R`, `GATE-L1.7B`, `L1.7C-0`, `L1.7C`, `L1.7D`, `L2.0`, `L1.7C-MATH`, `L1.7B-MAP`, `L1.7B-P23`, and `SPEC-ET-1`. Platform `GAME-UX-1` supplied shared skill-map runtime support; platform `GAME-UX-2` supplied source-controlled `1.1.1` checkpoint support. `L1.7C-MATH` restored scoped skill-tree math as primary `Rekenen`; `L1.7B-MAP` fixed checkpoint metadata and kept `targetReadinessEvidence: false`; `L1.7B-P23` accepted the stop/handoff because choice-only checks cannot honestly cover `1.1.2` calculation/index or `1.1.3` graph/table operation chains; `SPEC-ET-1` corrected the stable specs so exit tickets are target-equivalent proof tasks, not readiness-to-try checks. SYNC-4 makes the shared task-type UI part of the product end-state and adds the operational engine proof track. MTU-H4A prepared answer-form execution-planning evidence but authorizes no mutation or product use. Reference/product planning must not treat the lesson companion pipeline as scale-approved until `GATE-MTU-H4A`, `EX-LESSON-1`, `GAME-UX-3A`, `ENGINE-OP-1`, `SKILLMAP-OP-1`, `GRAPH-UX-2`, `MATH-UX-2`, `REASON-UX-2`, `GAME-ARCH-1`, `GATE-ENGINE-1`, lesson `L1.7B-Q2`, `GATE-L1.7B-Q2`, `REV-STD-1`, minimal curriculum-versioning readiness, and Scale Gate 1 resolve or are explicitly waived while preserving blocks on diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, and PV machine promotion. |
...
- `LESSON-SCALE-1` tracks the lesson-side pre-scale dependency: L1.7A rejected
  ...
  companion/product scaling must still wait for GATE-MTU-H4A, GAME-UX-3A,
...
| GATE-MTU-H4A-answer-form-cli-mutation-plan | before answer-form MTU lanes advance to an execution-packet sprint | yes | yes | active review gate for exact proposed IDs `A80`, `A81`, and `A96`-`A99`, ID-pressure decision, held graph/Type 4/analysis lanes, `bron` modifier boundary, EX overlay visibility, generator/exposure-block proof, and no mutation or product-use authority |
...
GATE-MTU-H4A answer-form CLI-mutation plan human review
...
Next action: run `GATE-MTU-H4A Answer-Form CLI-Mutation Plan Human Review`
after the H4A packet and cited evidence are committed and pushed to the normal
remote branch. The review must decide whether only a later bounded execution
packet may be prepared for the accepted H4A lanes. It must explicitly review
the proposed ID allocation `A80`, `A81`, and `A96`-`A99`, the invalid `A100`
constraint, the unused `A71` constraint, `A96` bereken, `A97`/`A98`/`A99`
explanation lanes, `A80` noem/geef-aan, `A81` bron as source-use modifier,
held graph/draw/shade, held Type 4 motiveer/classificatie, held
analysis/evaluation, q3/q15 EX overlays, command/rollback validation, and all
source/projection/product boundaries. No hand edits to `references/machine/`
or `references/external`, unit minting, candidate storage creation, candidate
writes, target-exercise mutation, generated projection refresh, lesson-output
mutation, diagnostics, adaptive routing, mastery/sequencing, student-facing AI,
summative use, PV projection, PV machine promotion, Scale Gate 1, or
student/product use are authorized. GAME-UX-3A remains the shared task-type UX
foundation dependency, but must not claim live answer-form MTUs or
target-exercise readiness until later governed source mutations and evidence
close.
