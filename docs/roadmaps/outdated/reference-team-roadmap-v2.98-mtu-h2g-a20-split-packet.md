...
Roadmap version: `v2.98-mtu-h2g-a20-split-packet`
...
|--------|------|-----------|---------------|
| GATE-MTU-H2G | A20 Split And Affected-Mapping Human Review | no | **ACTIVE OPERATIONAL NEXT ACTION.** Run the formal human review for `GATE-MTU-H2G-a20-split-replacement` only after the H2G packet, review packet, bundle URLs, sprint result, and cited evidence are committed and pushed to the normal remote branch. Review the `A20` usage classifications for `3.2.2`, `3.3.3`, `4.1.2`, and `GEN.A20`; decide whether later work should update/split/deprecate `A20`, add `A94`/`A95`, update affected mappings, and handle generator behavior. This gate itself authorizes no direct `A20` mutation, `A94`/`A95` minting, target-exercise mapping write, generator implementation change, PV projection, lesson output, candidate writes, CP-6/Year-1 closure, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV machine promotion, or student/product use. |
...
Next action: run `GATE-MTU-H2G-a20-split-replacement` after the H2G packet, review packet, bundle URLs, sprint result, and cited evidence are committed and pushed to the normal remote branch. MTU-H2F has executed the reviewed conditional lanes for `A12`, `A88`, `A89`, `A90`, `A92`, and `A93`; MTU-H2G has prepared the non-mutating `A20` split/replacement and affected-mapping packet. The gate must decide whether later work may update/split/deprecate `A20`, add `A94`/`A95`, update affected mappings for `3.2.2`, `3.3.3`, and `4.1.2`, and handle `GEN.A20`. No hand edits to `references/machine/` or `references/external/`, direct `A20` mutation, `A94`/`A95` minting, target-exercise mapping write, generator implementation change, candidate write, lesson-output mutation, target-exercise promotion, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or student/product use are authorized before a human closure explicitly names the next bounded authority. GAME-UX-2 has already supplied the bounded checkpoint engine support; lesson-side broad companion scaling remains blocked by the lesson review/Scale Gate path, while the reference-side active lane is now `GATE-MTU-H2G`.
