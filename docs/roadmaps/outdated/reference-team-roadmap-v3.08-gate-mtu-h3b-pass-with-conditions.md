...
Roadmap version: `v3.08-gate-mtu-h3b-pass-with-conditions`
...
|--------|------|-----------|---------------|
| MTU-H3C | Incidence Pass-Through Bounded CLI Execution | no | **ACTIVE OPERATIONAL NEXT ACTION.** Execute only the GATE-MTU-H3B authorized bounded command set for `D41`, `D42`, `D43`, `D45`, `D46`, narrowed `D07`, exact authored mapping patches for `3.1.1`, `3.1.2`, and `3.1.3`, and generated projection refresh only after authorized source mutations. Before execution, record the reviewed remote commit/hash, run clean-worktree or expected-local-files preflight, fresh-check `D41`/`D42`/`D43`/`D45`/`D46` absent, fresh-check `D44` absent and absent from the command set, fresh-check `D07`/`D05`/`A38`/`A41`/`A93`/`A15` present, print every extracted spec and mapping array, run `D07 --dry-run`, prove `A15` and hidden elasticity are removed from `D07`, decide or justify the `D42` zero-needs status, preserve `A93`, keep `D44` held, run the full validation stack, and prove no unintended diffs. No hand edits to `references/machine` or `references/external`, target-exercise promotion, candidate writes, lesson output, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV projection, PV machine promotion, or student/product use are authorized. |
