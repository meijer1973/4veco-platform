...
Roadmap version: `v3.00-mtu-h2i-a20-cli-execution-packet`
...
|--------|------|-----------|---------------|
| GATE-MTU-H2I | A20/A94/A95 CLI Execution Human Review | no | **ACTIVE OPERATIONAL NEXT ACTION.** Run the formal human review for `GATE-MTU-H2I-a20-cli-execution` only after the H2I execution packet, review packet, bundle URLs, sprint result, and cited evidence are committed and pushed to the normal remote branch. Review the exact `A20` unit-update command with `A2.11` retained, exact `A94`/`A95` unit-add specs and unit-add dry-run limitation, exact authored mapping patches for `3.2.2`/`3.3.3`/`4.1.2`, the generator route that moves current `GEN.A20` behavior to `GEN.A95` and blocks `GEN.A20` until a narrowed derive-both generator exists, rollback, validation, projection refresh guardrails, and no-exposure proof. This gate itself authorizes no direct `A20` mutation, `A94`/`A95` minting, target-exercise mapping write, generator implementation change, generated projection refresh, PV projection, lesson output, candidate writes, CP-6/Year-1 closure, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, PV machine promotion, or student/product use. |
