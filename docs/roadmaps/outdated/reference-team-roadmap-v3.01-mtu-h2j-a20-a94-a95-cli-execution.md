...
Roadmap version: `v3.01-mtu-h2j-a20-a94-a95-cli-execution`
...
|--------|------|-----------|---------------|
| MTU-H2J | A20/A94/A95 Bounded CLI Execution | no | **ACTIVE OPERATIONAL NEXT ACTION.** Execute only the GATE-MTU-H2I authorized bounded scope after final preflight: `A20` unit-update with `A2.11` retained, `A94` and `A95` unit-adds, exact authored target-exercise mapping patches for `3.2.2` and `4.1.2` while verifying `3.3.3` unchanged, generator route moving current `GEN.A20` behavior to `GEN.A95` and blocking `GEN.A20` until a narrowed derive-both generator exists, and generated projection refresh after source mutations. Use the reference CLI for machine-reference changes, print extracted specs/mapping arrays/generator patch before mutation, refresh generator-readiness/non-exposure evidence, and run the full validation stack. No hand edits to `references/machine/` or `references/external/`, target-exercise promotion, PV projection, PV machine promotion, lesson output, candidate writes, CP-6/Year-1 closure, diagnostics, adaptive routing, mastery/sequencing, student-facing AI, summative use, or student/product use is authorized. |
