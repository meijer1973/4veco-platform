...
| B2-2.2.3-HARDEN | Book 2 Paragraph 2.2.3 Inkomenselasticiteit En Kruiselingse Elasticiteit Quality Hardening | no | Planned after 2.2.2. Should handle income elasticity, cross-price elasticity, and classification language without overloading students. | Start only after 2.2.2 closes and notation/sign conventions remain stable. |
| B2-2.2.4-HARDEN-MIXED-1 | Book 2 Paragraph 2.2.4 Mixed Elasticity Practice | no | Planned after 2.2.1-2.2.3. Should reuse the `gemengde_opgaven` target standard for a source-heavy mixed elasticity section. | Start only after the theory paragraphs are stable enough for mixed transfer. |
| B2-2.2-TARGET-V5-PROMOTE | Book 2 Chapter 2.2 Target V5 Promotion | no | Candidate after 2.2.1-2.2.4 have current quality evidence. | Promote only records that satisfy the governed target-quality standard; do not carry missing core requirements as PASS WITH FLAGS. |
| B2-2.2-CHAPTER-QA-1 | Book 2 Chapter 2.2 Chapter QA | no | Candidate after the Chapter 2.2 paragraph sequence. Should inspect assembled chapter output, answer models, page rhythm, and rendering warnings. | Start only after Chapter 2.2 paragraph output exists and validators support chapter-level review. |
