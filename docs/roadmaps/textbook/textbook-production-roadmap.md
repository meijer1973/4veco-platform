...
Scope: textbook improvement and writing work that prepares, builds, verifies, and reviews printed textbook output in `../4veco-lessen`, with planning evidence and sprint records in this repository. `BOOK2-TARGET-AUTHORITY-REMEDIATION-1` is active under Issue #229 to repair all twelve Book 2 target records as one exact platform-only candidate package. Lesson writes, generated student output, owner approval, target integration, merge, and Phase B remain blocked. The approved outline stays `approved_with_holds` at semantic hash `69d803d2786e97bbd7519d2feed3ee29b79751b00a3c8a440432621927a13cde`; the Ei supersession and all twelve exact candidate bindings remain open until the owner decides the reviewed package.
...
|---|---|---|---|
| BOOK2-TARGET-AUTHORITY-REMEDIATION-1 | Book 2 Target Authority Remediation | no | Active Issue #229 Phase A. The platform-only twelve-record candidate and Ei correction passed specialist and finished-artifact review; structural lead review, exact-head CI, and owner decision remain pending. Lesson writes, target integration, owner approval, and merge remain blocked. |
...
Finish specialist and independent verification, publish the complete Issue #229 draft PR, obtain structural lead review and exact-head CI, then stop for the owner's decision on the exact twelve-record package hash. Do not edit lessons, generate output, integrate targets, or merge.
