...
async function runPage(cdp, file, scenario) {
  ...
  const checks = await evaluate(cdp, `(() => {
    ...
    const result = {
      ...
      rootTheme,
      horizontalOverflow,
      isParagraphLanding: (document.body.getAttribute('data-layout') || '').startsWith('paragraaf-'),
      hasResourceGrid: Boolean(document.querySelector(".resource-grid")),
      hasRouteSection: Boolean(document.querySelector(".route-section")),
      hasPrimaryRouteLayers: Boolean(
        document.querySelector('[data-route-layer="start"]') &&
        document.querySelector('[data-route-layer="learn"]') &&
