...
| What is the Book 2+ Part A exercise sequence and how is it guarded? | Start with `skills/econ-exercise-builder.md`, then `references/authored/didactiek-principes.md` for rationale and `BUILD-PARAGRAPH.md` for the build gate. Run `build-scripts/workflows/check-part-a-exercise-authoring-contract.js`; its mutation coverage is in `build-scripts/workflows/check-part-a-exercise-authoring-contract.test.js`. The guard is platform-only and does not retrofit frozen Book 1 output. |
