# Optional economic-graph construction recipes

Read only the section needed for an implementation example. The original v2
body is preserved below as reference material. Its fixed canvas, coordinates,
construction order, library calls and imperative wording describe that recipe;
they are not current workflow requirements.

[The current graph skill](../economic-graph.md) governs output requirements.
Equations and the precision reference take priority over example assumptions:
do not change supplied functions to force a positive supply intercept or U-shaped
cost curves, omit required axis units, or extend a line beyond its valid domain.
Running the verifier's own self-test does not validate a newly created figure.

Lookup sections below: canvas/rendering (Part 1), scaffold/geometry example
(Part 1B), palette (Part 2), graph examples (Part 3), typography (Part 4), QA
examples (Part 5), Word/PPTX sizing (Part 6).
